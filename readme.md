...
Minerva Archive (BitTorrent, no account), Internet Archive (chunked parallel HTTP, optional), local ISOs, XBLA, DLC, XBLIG, Game Archives, Retro ROMs (62 systems via EdgeEmu), multi-disc support (every disc of a release is queued together, and a rip that packs the whole release into one archive installs its install disc too, so the game arrives ready to play), GOD/XEX/content install layouts, FTP transfer with persistent retry, server queue management (the queue and any download in progress survive closing the app and resume where they stopped), and persistent logging.
