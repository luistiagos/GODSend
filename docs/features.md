...
- **Crash diagnostics on the console:** the `launch.ini` written by `infrastructure/readyToPlayConfiguration.ts` sets `Dumpfile = Usb:\crashlog.txt`. If DashLaunch's exception handler shows the *Fatal Crash Intercepted!* banner on the console, the exception text lands in `crashlog.txt` at the root of the prepared stick instead of going only to the UART. Ask users to attach that file when reporting a crash.
