...
- **How it works:** `/disc-info` uses verified disc-specific hints before download. Once the ISO exists, the server treats its XDVDFS layout as authoritative: populated `00000002`/`FFFFFFFF` installer trees select Content, while playable continuation discs remain GOD. Content discs often carry a generic placeholder Title ID (`FFED2000`) in `default.xex`; the real parent ID is read from the embedded STFS package before installation. Unknown titles therefore do not depend on a manually maintained name list.
  ...
  - **Demos, betas and trials are never grouped into the release.** A row such as *NBA 2K (USA) (Multiplayer Demo)* is a separate product, not a disc, so it is not enqueued alongside the retail game and does not count toward the disc set. It stays browsable and downloadable on its own — it is only excluded from the release grouping.
...
- **Crash diagnostics on the console:** the `launch.ini` written by `infrastructure/readyToPlayConfiguration.ts` sets `Dumpfile = Usb:\crashlog.txt`, so an exception caught by DashLaunch's handler lands on the prepared stick instead of going only to the UART. Two crash handlers run in this environment — DashLaunch's `exchandler` and Aurora's own, which writes `Aurora\Data\Logs\<ts>.crash.log` plus `.callstack` — and which one produced a dump identifies the handler behind a banner such as *Fatal Crash Intercepted!*. **Since v2.12.96 the app collects those dumps itself:** when the stick is plugged back into the PC and a preparation starts, `infrastructure/consoleCrashArtifacts.ts` reads `crashlog.txt` and `Aurora\Data\Logs\` before anything is formatted or written, records what it found in the session log, and attaches the contents to an error report. It honours the existing error-reporting kill switch (`errorReporting` in the app config, default on): with it off nothing leaves the machine, and the session log still names the files so they can be attached by hand. The stick is only read, never modified, and only if it carries this app's own `.xbox-downloader\ready-to-play-v<N>.marker` — an unrelated drive is left alone, and a folder redirected by a junction is refused rather than followed onto the local disk. The nine crash dumps that ship inside the BadAvatar package are matched against the manifest by size and SHA-256 and excluded, so another console's state is never reported as the user's reproduction. See [`docs/bugs/open/2026-09-05-fatal-crash-intercepted-pendrive-preparado.md`](bugs/open/2026-09-05-fatal-crash-intercepted-pendrive-preparado.md).
