...
* **`models/`**: Tipos puros de domínio (sem dependências externas):
  ...
  * `compat.go`: Tabela de compatibilidade de discos multi-disco (`discCompatTable`), busca `DiscCompat()` e o parsing de nomes que agrupa os discos de um mesmo lançamento (`ExtractDiscInfo`, `FindCompanionDiscs`). `discSubtitlePattern` casa **palavra-chave de papel do disco em qualquer posição** dentro do grupo entre parênteses/colchetes — não uma lista fechada de frases. Os catálogos trazem texto livre (`(Single-Player Campaign)`, `(Install-Multiplayer)`, `(Campaign Part 1 & Multiplayer)`, `[DLC]`) e uma lista exata deixava 52 lançamentos sem agrupar, ou seja, baixando um disco só. O outro lado é invariante: os grupos que **distinguem** releases (`(USA, Europe)`, `(En,Fr,De)`, `(Triple Pack)`, `(Volume 1)`) não podem ser removidos, senão dois produtos diferentes viram discos um do outro. Há teste para as duas direções; ver a entrada 2.12.86 do `CHANGELOG.md`. Um **segundo ramo** do padrão casa `play` ou `game` como **grupo inteiro** (`(Play)`, `(Game)`): sozinhas as duas palavras nomeiam papel de disco — o GTA V do Redump é `(Disc 1) (Install)` + `(Disc 2) (Play)`, e o ramo de palavra-chave tirava só a primeira — mas exigir o grupo inteiro preserva os nomes que apenas as **contêm** (`(Sega Game Toshokan)`, `(Game no Kanzume Vol. 1)`, `(Required for Play)`), que identificam produtos distintos. `MissingDiscNumbers()` diz quais discos do lançamento nenhum catálogo fornece e `DeclaredDiscCount()` lê o `of N` de **qualquer** linha do conjunto, não só da primária: o Redump declara a contagem numa linha e omite na outra, e sem isso o mesmo lançamento era completo ou incompleto conforme o disco clicado. Ver a entrada 2.12.87. `GuessTitleIDFromMultiDiscName()` casa cada termo como **palavra inteira** (`containsWords`): por substring, o "v" de "IV", "Rev 1" e "DVD1" mandava GTA IV e San Andreas para o GTA V. `mandatoryInstallDiscs` (`RequiresMandatoryInstallDisc`) lista os títulos cujo disco jogável não inicia sem o disco de instalação, **com o conjunto exato de pacotes**. Hoje só o GTA V (`545408A7`, quatro pacotes em `00000002`). Não acrescente um título sem o conjunto verificado: uma entrada sem lista de pacotes obrigaria a aceitar qualquer arquivo como instalação. Ver a entrada 2.12.102.
  * `game.go`: Enums de plataformas (`Platform`) e status de trabalhos (`JobStatus`).
* **`app/`**: Container de estado central:
  * `app.go`: Estrutura `App`, locks de concorrência (`sync.Map`, mutexes), logs formatados (`Logf`, `LogStatus`), lookup de instalações. `LogStatus` é o único ponto por onde passam **todas** as transições de estado de **todos** os pipelines: é ele que espelha a mudança no registro durável da fila (`persistJobState`) e é ele que anexa o aviso de `IncompleteRelease` quando o estado é `Ready` — assim nenhum caminho de entrega consegue reportar meia release multidisco como pronta. Não desvie um pipeline para gravar em `JobQueue` diretamente — a fila para de sobreviver ao fechamento do aplicativo sem que nada quebre visivelmente.
...
* **`infrastructure/`**: Adaptadores de efeitos colaterais:
  ...
  * `helpers/`: Funções utilitárias (cálculo de IP, sanitização de nomes, inspeção de cabeçalhos XDVDFS).
    * `multi_disc.go`: Inventário de um rip multidisco já extraído. `FindCompanionContentPayloads` acha as árvores `Content/0000000000000000/<TitleID>/<tipo>` que ficam **fora** da pasta jogável — o disco de instalação que vem no mesmo arquivo — e `IsGenericDiscFolderName` reconhece o marcador de disco (`Disc2`, `DVD 1`, `Game Disc`) que o rip usa como nome da pasta raiz. O TitleID sai do cabeçalho do pacote e não do nome da pasta: instalador de fábrica endereça a própria árvore como `FFED2000`, e pacotes assinados sob `SystemTitleID` (Kinect, voz, dashboard) são descartados. `InstallPackageProblem`/`InstallSetProblem` conferem um conjunto de instalação pacote a pacote: a identidade vem do cabeçalho (TitleID em `0x360`, tipo em `0x344`), nunca do nome do arquivo, e o truncamento vem dos tamanhos que o cabeçalho STFS declara. Em todo pacote retail LIVE/PIRS conferido, o content size (`0x34C`) é exatamente o tamanho do arquivo menos o cabeçalho alinhado a 4 KiB. Homebrew e CON deixam esse campo zerado e só contam com o piso por blocos alocados. `PackageProbe` abstrai a leitura para servir tanto a disco local quanto a FTP.
* **`services/`**: Camada de aplicação e regras de negócio:
  * `cache/`: Serviços de cache para IA (`ia.go`), Minerva (`minerva.go`) e ROMs (`rom.go`).
  * `pipeline/`: Pipeline de conversão e transferência (`ProcessLocalISO`, `ProcessGame`, `FinalizeGOD`, `digital.go`, `minerva.go`, `rom.go`, `ini.go`). O nome da pasta de destino de uma instalação XEX **tem** de passar por `helpers.XEXFolderName()`, que acrescenta o TitleID — e não pode ser só o `filepath.Base(xexFolder)` que vem do arquivo baixado. Rips de jogos de dois discos trazem a pasta raiz chamada `Disc2`, então sem o TitleID títulos diferentes resolvem para o mesmo destino e, como `copyTreeLocal` mescla no que já existe, o segundo sobrescreve o `default.xex` do primeiro **em silêncio**, com as duas tarefas relatando sucesso. A composição fica nos seis pontos onde `folderName` é derivado, não nas funções de gravação, para valer também no FTP (`ftp/client.go` monta o destino com o mesmo nome). Duas condições da função não são óbvias e não devem ser removidas: ela **ignora `helpers.SystemTitleID`** (`FFFE07DF`), porque `FindTitleIDInDir` varre em ordem lexical e os pacotes Kinect embutidos no jogo — `Database.xmplr` e afins — vêm antes de `default.xex`, o que produziria uma pasta que o próprio scan de jogos instalados descarta como dado de sistema; e **corta o nome em 42 caracteres** (limite FATX do HD interno), preservando o TitleID, porque `TransferXEX` descarta o resultado de `MkdirAll` e um nome recusado vira job de FTP falhando igual em toda retentativa. Ver a entrada 2.12.81 do `CHANGELOG.md` antes de simplificar essa nomenclatura. O nome passa hoje por `xexDestinationName()` (`companion_content.go`), que envolve `XEXFolderName` e troca o marcador de disco pelo título da release antes de acrescentar o TitleID — sem isso o GTA V entra na biblioteca chamado "Disc2". E **todo** caminho de instalação XEX a partir de um arquivo extraído tem de chamar `installCompanionDiscContent()` antes de gravar o jogo: `FindXEXFolder` devolve só a primeira pasta com `default.xex`, então sem essa chamada o disco de instalação que veio no mesmo arquivo é apagado com o scratch e o jogo chega ao console sem dar boot. A ordem importa — conteúdo primeiro, jogo depois — para que uma falha pare o job enquanto o fallback ainda pode procurar uma release que caiba no destino. Ver a entrada 2.12.85 do `CHANGELOG.md`. A entrega do disco **jogável** de um título de `mandatoryInstallDiscs` passa por `requireInstallContent()`, que exige o conjunto inteiro no destino. Fora da pasta XEX ou GOD do arquivo, que já é coberta por `installCompanionDiscContent()`, ela é chamada em `convertAndFinalizeGODResilient()`, antes do laço GOD de `ProcessGameWithErr()` e no ramo XEX de `ProcessLocalISO()`. Qualquer caminho novo que entregue um disco jogável precisa chamá-la também. O **disco de instalação nunca pode ser cobrado pelo próprio conteúdo**: a exigência só vale para uma pasta com `default.xex` ou estrutura GOD (`playableTitleID`) e, nos caminhos de ISO, só depois de `resolveISOInstallType()` ter classificado o disco. Chamá-la antes, como a primeira tentativa desta correção fazia, bloqueia a única entrega que grava o conteúdo. Exceção conhecida: com o tipo XEX forçado (os discos companheiros herdam o tipo do primário em `enqueueCompanions`), a ISO do Disco 1 vira pasta XEX sem passar por `resolveISOInstallType()` e cai na exigência. Esse caminho já não instalava o conteúdo do Disco 1, e distinguir o disco pelo número gravado no `default.xex` arriscaria liberar justamente um rip do disco jogável. O TitleID vem do executável, nunca do nome. Só um destino **lido e incompleto** falha o job. Destino ilegível, pacote manual e tarefa do Disco 1 da mesma release ainda na fila (`pendingInstallDiscJob`) entregam com aviso em `IncompleteRelease`, porque cada disco Redump é um job separado e os jobs rodam em qualquer ordem. Pelo FTP (`ftpInstallSetProblem`), a leitura segue as regras que `services/content` aprendeu com o Aurora: `CWD` e depois `LIST("")`, `RETR` pelo nome, pausa entre canais de dados e prazo total (`ftpInstallCheckLimit`). **Não troque por `List(caminhoAbsoluto)`**: o Aurora ignora o caminho, a listagem volta sem os pacotes e o job falha com um falso `release incompleta` logo depois de gravar 8 GB. A checagem roda com a fila de jobs e o slot FTP do console presos, então pedido sem resposta vira "não conferido", nunca falha nem espera sem prazo. Ver a entrada 2.12.102. **Toda** chamada de `extractArchiveResilient`/`extractISOResilient` tem de passar o erro por `invalidateDownloadedArchiveOnCorruptExtract()` (`archive_cache.go`) e chamar `clearCorruptArchiveRedownloadMarker()` no sucesso. Em modo local o arquivo de origem e retido de proposito fora do scratch (`.source*` em `GetReadyDir()`, torrent em `torrentDir`), e os dois caches o reaproveitam sem rebaixar: o de HTTP confere o SHA-256 que ele mesmo gravou do arquivo em disco, e o de torrent confere so o tamanho. Sem essa chamada, bytes que chegaram corrompidos falham identico em toda tentativa, para sempre, sem nenhum request — foi o que prendeu `Lego Batman 1` por quatro execucoes em dois dias, e o que sobreviveu em `rom.go` e nos quatro pontos do `minerva.go` depois que a 2.12.94 corrigiu so os outros. A limpeza e estreita de proposito: `isArchiveIntegrityError` e `isLikelyLocalStorageError` nunca podem se sobrepor (disco cheio nao pode apagar um download integro de varios GB). Ver as entradas 2.12.94 e 2.12.97 do `CHANGELOG.md`.
...
* **`interfaces/http/`**: Camada HTTP/REST:
  ...
  * `/browse/releases` (`handleBrowseReleases`): agrupamento multidisco do catálogo de uma plataforma, servido ao renderer. Usa `companionCatalog()` — a mesma união de caches que o `enqueueCompanions` varre — e `models.FindCompanionDiscs`, para que a tela nunca liste um disco que a fila não buscaria **nem esconda um que ela buscaria**. Não filtre por `IsMultiDisc` antes de agrupar: `FindCompanionDiscs` casa só pelo título do lançamento, então uma linha sem `(Disc N)` — `Call of Duty - Advanced Warfare (USA, Europe) (Install Disc)` e o `(Game Disc)` ao lado — é disco que o `/trigger` busca, e o filtro fazia a tela prometer menos do que a fila entrega. Há teste comparando contra o `FindCompanionDiscs` sobre o catálogo inteiro.
...
* **`services/`**: Regras de aplicação no Node.js:
  ...
  * `autoUpdateService.ts`: Atualização in-app (manifesto, download com SHA-256, troca do `.exe`). **A troca espera processos, nunca um relógio**: no portable o alvo é o próprio launcher (`PORTABLE_EXECUTABLE_FILE`), que só sai depois do Electron fechar **e** de apagar a pasta extraída de ~1,3 GB — um orçamento fixo de ~11 s falhou em 11 de 11 aplicações observadas. `applyUpdateAndRestart()` grava `<userData>/update-pending.json`, o script grava `update-result.txt`, e `checkPendingUpdateResult()` (no boot) reporta à telemetria e avisa o usuário quando a versão não mudou. Nunca sobrescrever o exe extraído em `%TEMP%` (`resolveReplaceTarget`). Canais: `update:check`, `update:download`, `update:cancel-download`, `update:apply` (`{ ok:false }` precisa ser lido pelo renderer), `update:take-apply-failure`, `update:show-downloaded-file`, `update:dismiss-version`, `update:get/set-auto-check`.
  ...
  * `badAvatarUsbService.ts`: Orquestrador de criação do pendrive BadAvatar USB (formatador FAT32 + gravação transacional do BadStick). `listFat32UsbDrives()` é a lista que a UI consulta em polling (Home a cada 5 s, contador de jogos a cada 7 s): no Windows ela é reaproveitada até a letra ou o serial (`stat().dev`) de um volume mudar, com 15 s de reenumeração depois de cada mudança e `freeBytes` relido por `statfs`. **Polling de UI não pode chamar `enumerateSafeWindowsUsbDevices()` direto** — cada chamada abre dois ou três PowerShells, e em algumas máquinas cada um pisca uma janela de terminal (v2.12.98). Quem altera o volume sem mudar o serial (reparo) chama `invalidateUsbDriveListCache()`; ação explícita do usuário ("Atualizar", verificação manual) passa `{ fresh: true }` a `tools:badavatar-list-drives` — nunca um poll. Só `ENOENT` tira uma letra da assinatura: volume RAW tem letra e `stat` falhando, e precisa contar. Cada dispositivo da lista traz `preparedState` (`sem-preparo | jogos-apenas | preparado-rgh | preparado-bloqueado-lt`), de `preparedDeviceState.ts`, e **nunca mais um booleano "já preparado"**: a única prova de que um pendrive serve a console travado ou LT é o **perfil do exploit** (`Content/<XUID>/…`, lido do manifesto ativo por `exploitProfileFiles()` — jamais de uma constante, porque o XUID muda num `payload:update`). `Content/0000000000000000`, `Aurora/`, `launch.ini`, `Games`, `FSD`, `Freestyle` e `default.xex` **não provam nada**: o baixador de jogos, um preparo em modo RGH e pacotes de terceiros produzem cada um deles. Invariante de UI que vale para qualquer tela nova: **nenhum texto pode afirmar que existe desbloqueio, e nenhuma navegação pode oferecer pular a preparação, fora de `canSkipPreparation()`** — o defeito anterior mandava clientes de console travado direto ao catálogo com um pendrive que nunca abriria a Aurora (v2.12.99, `docs/bugs/closed/badavatar-usb-service-declara-ja-preparado-sem-conferir-o-perfil-do-exploit_2026-09-23T14-08.md`).
  * `preparedDeviceState.ts`: Responde **o que o dispositivo tem**, não "tem alguma pasta de Xbox": `detectPreparedDeviceState()` devolve `sem-preparo | jogos-apenas | preparado-rgh | preparado-bloqueado-lt`. `preparado-bloqueado-lt` exige o perfil do exploit presente **e com o tamanho do manifesto**; `preparado-rgh` é o marcador `.xbox-downloader/ready-to-play-v<N>.marker` (qualquer versão) ou `Aurora/default.xex` + `launch.ini` sem o perfil. Confere tamanho, **não SHA-256**, de propósito: isto decide qual tela aparece, não se a gravação será reaproveitada (quem confere hash é o escritor transacional), e hashear o pacote a cada varredura de 5 s leria o pendrive inteiro com o app parado na Home. Perfil vazio (manifesto ilegível) **nunca** produz `preparado-bloqueado-lt`.
  * `preparedUsbDetection.ts`: Sem imports do Electron nem de `fs`, por isso é importado direto pelo renderer e testável em `tests/unit/`. `readPreparedUsbDetection()` reduz a lista de dispositivos ao estado mais preparado, `canSkipPreparation()` é a **única** porta do caminho "pular a preparação e ir ao catálogo", e `nextStepAfterPreparedUsbScan()` só desvia o assistente com prova no dispositivo — o caso RGH depende de uma declaração do usuário, lida no clique dele, não numa varredura de 5 s que trocaria a tela debaixo de quem acabou de escolher o modo do console.
  * `preparedDeviceCopy.ts`: O texto que o usuário lê por estado, num só lugar, compartilhado por `HomePage.tsx` e `BadAvatarUsbPage.tsx`. **Só `preparado-bloqueado-lt` pode afirmar que existe desbloqueio ou convidar a pular a preparação** — `tests/unit/preparedDeviceCopy.test.cjs` reprova qualquer texto novo que quebre isso.
...
* **`ipc/`**: Manipuladores IPC expostos ao React (`configHandlers`, `xboxFtpHandlers`, `auroraLibraryHandlers`, `auroraAssetHandlers`, `browseHandlers`, `toolsHandlers`, `contentHandlers`, `saveHandlers`, `badAvatarHandlers`, `updateHandlers`).
  * `browseHandlers.ts`: `browse:get-games`, `browse:get-release-groups`, `browse:queue-game`, `browse:get-disc-info`, `browse:fetch-cover` e a fila (`xbox:get-queue`, `xbox:remove-queue-item`, `xbox:retry-queue-item`). `browse:get-release-groups` traz de `GET /browse/releases` quais linhas do catálogo são discos do mesmo lançamento — **o renderer não deriva isso do nome**; ver a nota de `renderer/`.
* **`infrastructure/`**:
  * `fat32Format.ts`: Formatação FAT32 multiplataforma para drives USB grandes (Ridgecrop `fat32format.exe` no Windows, `newfs_msdos`/`diskutil` no macOS, `mkfs.vfat`/`mkfs.fat` no Linux). Os quatro caminhos de `diskpart` usam **`convert mbr`, e isso é deliberado** — o Xbox 360 lê FAT32 em MBR; trocar por GPT devolveria capacidade no Windows e tornaria o disco ilegível no console. MBR e FAT32 limitam um volume a 2 TiB cada um, então um HD de 4 TB termina com ~2 TB e o resto fica inendereçável: **é comportamento esperado, não bug a corrigir**. Acima de 32 GB o caminho principal formata a partição existente e só cai no `diskpart` quando o `fat32format` falha. A partição que o `diskpart` entrega ao `fat32format` é criada **sem sistema de arquivos**, e isso é deliberado — o `fat32format` grava setores direto no volume e o Windows só permite escrita bruta onde nenhum sistema de arquivos está montado; a pré-formatação `format fs=ntfs quick` que existiu entre a 2.12.62 e a 2.12.87 (posta ali só para suprimir o modal *“Formate o disco na unidade X:”*) devolvia exatamente o estado que faz a gravação falhar com `GetLastError()=5`. **Não recoloque o NTFS antes do `fat32format`** — leia a entrada da 2.12.88 no `CHANGELOG.md`. Abaixo de 32 GB o NTFS continua, porque ali quem formata é `Format-Volume`/`format.com`, que são API de sistema de arquivos e não sofrem o bloqueio. Falha depois de removermos o sistema de arquivos devolve a partição a NTFS, para o disco não ficar RAW. O **acesso controlado a pastas** do Defender (proteção contra ransomware) barra essa mesma escrita bruta mesmo com a partição sem sistema de arquivos, e o sintoma é o mesmo `GetLastError()=5`: `Unblock-Fat32RawWrite`/`Restore-Fat32RawWriteProtection` põem o `fat32format.exe` em `ControlledFolderAccessAllowedApplications` só nos modos que bloqueiam (1 e 3) e o retiram num `finally`, que roda antes do `exit 1` do `catch`. **Não troque por `Set-MpPreference`** — desligar a proteção inteira do usuário para formatar um pendrive é desproporcional. Achar o nosso `fat32format.exe` já na lista faz o script **adotar** a entrada (`$script:cfaAllowlistAdded = $true`) e retirá-la no fim, e isso é deliberado: o flag vive só no processo elevado, então uma execução morta antes do `finally` deixaria a sobra na lista e toda execução seguinte a leria como preexistente, tornando permanente a exceção de um gravador bruto de disco na proteção contra ransomware. **Não volte para “se já está na lista, não mexer”.** Um disco acima de 2 TiB recebe faixa de aviso na seleção (`BadAvatarUsbPage.tsx`, constante `MBR_MAX_VOLUME_BYTES`) — **avisa, não bloqueia**: o teto ficou deliberadamente fora de `assessDeviceSafety`, cujos códigos são todos impeditivos, porque 2 TB funcionam. Ver [`docs/CAPACIDADE-E-ESPACO.md`](docs/CAPACIDADE-E-ESPACO.md#teto-de-2-tib-por-volume).
  * `readyToPlayConfiguration.ts`: Gera o `launch.ini` do DashLaunch (inclusive `Dumpfile = Usb:\crashlog.txt`, que preserva no pendrive a exceção que o DashLaunch interceptar, em vez de mandá-la só para a UART), o marcador e o script Lua carregado pelo Aurora. `READY_TO_PLAY_CONFIGURATION_VERSION` precisa subir sempre que o conteúdo gerado mudar, senão o diário transacional do pendrive rejeita o novo plano. O Lua é gravado em `Aurora/User/Scripts/Content/Filters/` e roda na fase `ContentScripts` do boot; o `Aurora.Restart()` no fim dele é **deliberado** — os scan paths só valem a partir da inicialização seguinte. Esse reinício é de **uma vez só, e o limite é obrigatório**: antes de reiniciar, o hook grava `.xbox-downloader/ready-to-play-v<N>.restart` no pendrive e confere que o arquivo ficou lá; se o cadastro voltar a acontecer no boot seguinte, a marca já existe e ele desiste do reinício em vez de repeti-lo a cada boot — o laço que o usuário vê como console reiniciando sozinho ao inserir o pendrive. Um boot que encontra o banco já correto **apaga** a marca, devolvendo o direito a um reinício caso o Aurora recrie o banco depois. Pelo mesmo motivo, `ensureScanPath` compara `deviceid` por `normalizedSerial`, não por igualdade crua: o mesmo pendrive aparece em `FileSystem.GetDrives` como `Usb0:` com o serial físico e como `Game:` com esse serial prefixado por `_`, e comparar cru faria a linha já gravada parecer de outro aparelho. Antes de mexer nesse script, leia [`docs/READY-TO-PLAY-AURORA.md`](docs/READY-TO-PLAY-AURORA.md), que descreve a sequência de boot, e use [XboxUnity/AuroraScripts](https://github.com/XboxUnity/AuroraScripts) como referência de API — [`docs/reference/aurora.md`](docs/reference/aurora.md) é condensada e não cobre `Sql.*` nem `Aurora.Restart`.
  * `consoleCrashArtifacts.ts`: Lê do pendrive preparado, **sem alterá-lo**, os registros que o console deixou de uma falha: o `crashlog.txt` da raiz (o `Dumpfile` do DashLaunch) e o conteúdo de `Aurora/Data/Logs`. `collectConsoleCrashArtifacts()` é chamado por `fixedBadAvatarPreparationService.ts` **antes da formatação**, e a posição é obrigatória: formatar apaga o vestígio, e essa é a única janela em que ele existe quando "Formatar antes" está marcado. Por isso o manifesto é carregado antes do passo de formatação. Essa mesma posição traz duas exigências de segurança, porque o conteúdo lido **sai da máquina** pela telemetria e a varredura roda antes da recusa de sistemas de arquivos que não sejam FAT32 — ou seja, um volume removível em NTFS chega até ela. **(a)** Nada é lido sem o marcador `.xbox-downloader/ready-to-play-v<N>.marker` (qualquer versão, porque quem tem um dump para entregar preparou o pendrive numa versão anterior): `crashlog.txt` nunca está no manifesto, então a conferência por hash jamais o descartaria, e sem essa porta um pendrive alheio com um arquivo desse nome teria o conteúdo enviado. **(b)** `resolveRealDirectory()` confere **cada ancestral**, não só a folha — um `mklink /J` numa pasta do dispositivo faria a leitura cair no disco local. `foreignConsoleState.ts`, que percorre a mesma árvore, recusa junction e link pelo mesmo motivo; não relaxe nenhuma das duas. **Não devolva um arquivo sem conferir o manifesto** — o pacote BadAvatar traz nove crash dumps de outro console, e reportá-los como reprodução do usuário foi exatamente o que atrasou o diagnóstico de `docs/bugs/closed/2026-09-05-fatal-crash-intercepted-pendrive-preparado.md`; uma cópia ainda idêntica em tamanho e SHA-256 é contada em `bundledCopiesSkipped` e não vira artefato. Um arquivo do pacote que o console alterou (o `debug.log`, que recebe as linhas do hook a cada boot) deixa de conferir e **volta como artefato**, que é o comportamento desejado. `debug.log`/`debug.log.last` são cortados pelo **fim** (cronológicos), os demais pelo **começo** (o endereço e o módulo da exceção abrem o dump). Limites abaixo dos da telemetria: 64 KB por arquivo, 512 KB no total, 12 artefatos, os mais recentes primeiro.
  * `exploitProfile.ts`: `isExploitProfilePath()` — o perfil do exploit é `Content/<XUID>/…` com XUID diferente de `0000000000000000` (essa é a pasta de conteúdo do console, que o baixador de jogos cria sozinho). É a única parte do pacote que o modo RGH **não** grava, e por isso a única prova de que o `ABadAvatar` vai aparecer num console travado ou LT. O XUID sai sempre do manifesto ativo (`exploitProfileFiles()`), **nunca de uma constante**: ele pertence ao pacote e muda num `npm run payload:update`.
  * `preparationReceipt.ts`: Grava `.xbox-downloader/preparo.json` com modo, data, versão do app e release do pacote — o que o marcador de uma linha nunca guardou, e sem o que ninguém consegue responder depois **como** um pendrive foi preparado. Escrito **fora da escrita transacional e depois dela**, e isso é obrigatório: `planHash` deriva do conteúdo do plano enquanto o `transactionId` vem de `bundleSha256 + fingerprint + scope`, então qualquer byte volátil dentro do plano faz toda retomada interrompida falhar em `verifyTransactionJournalAgainstPlan`. **Não mova o recibo para dentro do marcador nem do plano.**
...
* **`renderer/`**: Interface visual construída em React 19, Vite e TailwindCSS (páginas `HomePage`, `LibraryPage`, `QueuePage`, `SettingsPage` e overlays).
  * `components/BrowsePage.tsx`: **Não analisa nome de disco.** Número do disco, título do lançamento e quais nomes pertencem a ele vêm de `browse:get-release-groups`, que roda o mesmo agrupamento que o `/trigger` usa para enfileirar companheiros. Havia aqui um `parseDiscGame` próprio, com uma lista fechada de papéis que exigia a palavra `Disc` dentro do grupo e não olhava colchetes: sobre os catálogos empacotados ele deixava 172 lançamentos com um disco só onde o backend deixava 85, e o botão *"Baixar todos os N discos"* obedece a este lado. Não recrie esse parser — não há modo offline que o justifique, o catálogo na tela veio do mesmo servidor. `getBaseTitle`/`getComparisonKey` continuam locais porque agrupam **versões** do mesmo jogo para exibição, o que não tem relação com disco. Quando `missing_discs` não está vazio, o diálogo de download exige confirmação explícita antes de liberar o botão. Ver a entrada 2.12.87 do `CHANGELOG.md`.
