...
* **`infrastructure/`**:
  ...
  * `windowsSystemExecutables.ts`: Resolve `powershell.exe`, `cmd.exe`, `net.exe` e `chkdsk.exe` por caminho absoluto sob `%SystemRoot%\System32`. **Nunca passe o nome simples para `spawn`/`spawnSync`** — o nome é resolvido pelo `%PATH%`, e em instalações com o PATH truncado ou "debloatado" todo `spawn` falha com `spawn powershell.exe ENOENT`. `powerShellExe()` para o interpretador, `system32Exe()` para os demais, `POWERSHELL_EXE_PS_EXPRESSION` para o `Start-Process` aninhado da elevação e `isExecutableNotFound()`/`powerShellMissingMessage()` para traduzir o ENOENT restante.
