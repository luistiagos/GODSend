...
* **`infrastructure/`**:
  ...
  * `windowsSystemExecutables.ts`: Resolve `powershell.exe`, `cmd.exe`, `net.exe` e `chkdsk.exe` na ordem `%PATH%` → `%SystemRoot%\System32` → nome simples. **Nunca passe o nome simples direto para `spawn`/`spawnSync`** — aí o `%PATH%` é a única fonte, e em instalações com o PATH truncado ou "debloatado" todo `spawn` falha com `spawn powershell.exe ENOENT`. `powerShellExe()` para o interpretador, `system32Exe()` para os demais, `isExecutableNotFound()`/`powerShellMissingMessage()` para traduzir o ENOENT restante. O `Start-Process` aninhado da elevação **não** refaz a busca: `buildElevationScript()` (em `fat32Format.ts`) recebe o interpretador já resolvido pelo pai. Com o `%PATH%` na frente, um `powershell.exe` plantado numa pasta anterior da lista vence — exposição aceita deliberadamente; ver a entrada 2.12.78 do `CHANGELOG.md` antes de mexer nessa ordem. `describeWindowsExecutableEnvironment()` produz a linha de diagnóstico anexada ao log e à telemetria quando a enumeração de USB falha (PowerShell no disco, PATH com/sem `System32`, tamanho do PATH, `arch`) — com a pasta do perfil mascarada, porque o log inteiro é enviado a um serviço externo.
