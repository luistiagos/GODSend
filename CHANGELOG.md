...
- **O disco jogável do GTA V deixa de ser entregue sem o conteúdo de instalação do Disco 1 (`models/compat.go`, `infrastructure/helpers/multi_disc.go`, `services/pipeline/companion_content.go`, `services/cache/huggingface.go`) - v2.12.102**:
  ...
  - **Catálogo.** A URL incompleta é descartada no rebuild e ao carregar um cache salvo, e "GTA 5" passa a apontar para `Grand.Theft.Auto.5.EUR.X360-ZTM.rar` (Archive.org; exige conta logada, como as demais releases ZTM do catálogo). A descrição do item confirma a árvore `hdd1/content/0000000000000000/545408A7` no arquivo.
  - **Pipeline.** Toda entrega do disco jogável de um título com disco de instalação obrigatório (pasta XEX ou GOD de um arquivo, ISO→GOD, ISO→XEX local) exige o conjunto inteiro no destino. Cada pacote é conferido pelo nome, com TitleID e tipo de conteúdo lidos do cabeçalho e o tamanho comparado ao declarado, o que pega até 1 byte a menos nos pacotes retail. Um arquivo qualquer na pasta, um pacote de outro título ou um conjunto parcial não bastam mais. Um conjunto parcial vindo no arquivo não é gravado por cima do destino, e uma segunda cópia do mesmo conjunto no arquivo (a do disco e a de `hdd1`) não é gravada de novo. Sem o conjunto, o job falha com `release incompleta` e o fallback de provedores segue adiante. A entrega continua, com aviso, quando o destino não pode ser lido, quando é pacote manual ou quando a tarefa do Disco 1 da mesma release ainda está na fila: cada disco Redump é um job separado e eles rodam em qualquer ordem.
  - **No console por FTP, só conta o que o console mostra.** A leitura segue o que `services/content` já tinha aprendido com o Aurora: `CWD` na pasta e `LIST` sem caminho, porque o Aurora ignora ou recusa caminho absoluto, `RETR` pelo nome, pausa entre canais de dados e prazo total de 30 s. Pasta inexistente (550 no `CWD`), pacote fora da listagem e cabeçalho de outro título ou truncado falham o job. Qualquer pedido sem resposta deixa a conferência em aberto e a entrega segue com aviso, sem prender a fila de jobs nem o console. No modo local, outro pendrive montado na mesma letra também deixa a conferência em aberto.
  - **O disco de instalação nunca é cobrado pelo próprio conteúdo.** A exigência só vale para uma pasta com `default.xex` ou estrutura GOD e, nos caminhos de ISO, só depois de `resolveISOInstallType` classificar o disco como jogável. O TitleID vem do executável, nunca do nome do catálogo. A exceção é o tipo XEX forçado, que extrai a ISO do Disco 1 sem classificá-la. Esse caminho já não instalava o conteúdo.
  - **O palpite de TitleID pelo nome passou a casar palavras inteiras.** Por substring, o "v" de "IV", de "Rev 1" e de "DVD1" levava GTA IV, San Andreas e Advance GTA para o GTA V. A mesma troca corrigiu quatro palpites antigos: um addon do Forza 3 tomado pelo Forza 4 por causa do "4" do hexadecimal, BioShock e Borderlands GOTY tomados pelas sequências por causa do "2" de "DVD2", e Mafia III tomado por Mafia II.
  - MGS V e Alien: Isolation ficaram fora da lista de títulos obrigatórios. Sem o conjunto de pacotes verificado, a checagem teria de aceitar qualquer arquivo, que é justamente o defeito corrigido aqui.
  - Regressões: `services/pipeline/install_content_test.go`, `services/pipeline/install_content_ftp_test.go` (servidor FTP falso que se comporta como o Aurora, incluindo `LIST` sem canal de dados e `RETR` que nunca termina), `models/compat_test.go` e `services/cache/refresh_safety_test.go`.
- **Encerramento formal e diagnóstico comprovado de `Fatal Crash Intercepted!` (`docs/bugs/closed/2026-09-05-fatal-crash-intercepted-pendrive-preparado.md`)**:
  - Fecha formalmente o bug com base nos artefatos de telemetria coletados de consoles reais em produção (relatórios 8238, 7734, 7704 e 7608 via `collectConsoleCrashArtifacts`).
  - O pipeline de inicialização, DashLaunch e o hook `XboxCompanionReady.lua` foram comprovados 100% estáveis na carga do Aurora (zero ocorrências de `crashlog.txt` na raiz dos pendrives e logs de inicialização completos).
  - Os crash dumps isolados observados em sessões posteriores decorreram de fatores não relacionados (abertura física da bandeja de DVD pelo usuário e lançamento de contêiner multidisco danificado), e os 26 arquivos de estado estranho do console doador foram eliminados na v2.12.74 (`isForeignConsoleStatePath`).

## [2.12.101] - 2026-09-24

### Added
- **O pendrive preparado entrega sozinho o vestígio da falha do console (`infrastructure/consoleCrashArtifacts.ts`, `services/fixedBadAvatarPreparationService.ts`) - v2.12.96**:
  - Trabalha o passo bloqueante de `docs/bugs/closed/2026-09-05-fatal-crash-intercepted-pendrive-preparado.md`. A 2.12.68 fez o vestígio **existir** — `Dumpfile = Usb:\crashlog.txt` no `launch.ini`, em vez de despejar a exceção só na UART —, mas a chegada dele continuava dependendo de pedir a quem reportou que localizasse dois caminhos à mão e os enviasse. Nenhuma reprodução real chegou assim, e sem dump não dá para dizer sequer **qual** manipulador desenha o banner *Fatal Crash Intercepted!*, quanto mais qual exceção ocorreu.
  - **Agora quem lê é o aplicativo.** Quando o pendrive volta ao PC e uma preparação começa, `collectConsoleCrashArtifacts()` recolhe o `crashlog.txt` da raiz (caminho DashLaunch) e o conteúdo de `Aurora\Data\Logs` (caminho Aurora — `.crash.log`, `.callstack`, `debug.log`), registra o achado no log da sessão e anexa o conteúdo a um reporte de telemetria. O dispositivo é apenas lido: nada é movido, alterado ou apagado.
  - **A leitura roda antes da formatação, e a posição é o ponto todo.** Quem marca **Formatar antes** apagaria o dump antes de ele ser lido, e essa é exatamente a situação de quem está tentando reproduzir a falha. Por isso o manifesto do pacote passou a ser carregado antes do passo de formatação — falhar num manifesto inválido antes de apagar o disco do usuário também é a ordem certa por si só.
  - **Uma cópia do pacote nunca é reportada como reprodução do usuário.** O BadAvatar foi capturado de um console real e traz nove crash dumps daquela máquina; confundi-los com o dump de quem reportou foi o que atrasou este diagnóstico desde o começo. Cada candidato é conferido contra o manifesto por tamanho e SHA-256: o que ainda confere é contado em `bundledCopiesSkipped` e descartado. A checagem é refeita aqui em vez de confiar na quarentena da 2.12.74, que só roda durante uma preparação e só move o que ainda confere.
  - **O arquivo que o console alterou é recolhido de propósito.** O `debug.log` do pacote recebe as linhas do hook a cada boot, então deixa de conferir com o manifesto — e é ele que separa "a divergência de `deviceid` continua viva" (`caminhos recadastrados sem reiniciar`) de "o console está reiniciando sozinho". Por isso `debug.log`/`debug.log.last` são cortados pelo **fim**, e os dumps pelo **começo**, onde ficam o endereço e o módulo da exceção.
  - **Duas portas, porque o conteúdo lido sai da máquina.** Como a varredura roda antes da recusa de sistemas de arquivos que não sejam FAT32, um volume removível em NTFS chega até ela. (a) Nada é lido sem o marcador `.xbox-downloader\ready-to-play-v<N>.marker` — `crashlog.txt` nunca está no manifesto, então a conferência por hash jamais o descartaria, e sem essa porta um pendrive alheio com um arquivo desse nome teria o conteúdo enviado; aceita-se qualquer versão do marcador porque quem tem um dump para entregar preparou o pendrive numa versão anterior. (b) Cada pasta do caminho é conferida antes de ser aberta, então uma junction apontando para fora do dispositivo é recusada em vez de seguida até o disco local — a mesma defesa que `foreignConsoleState.ts` já fazia na mesma árvore.
  - Tetos abaixo dos da telemetria (64 KB por arquivo, 512 KB no total, 12 artefatos, os mais recentes primeiro) e respeito ao kill-switch `errorReporting`: desligado, nada sai da máquina e o log da sessão continua nomeando os arquivos. Diagnóstico nunca interrompe uma preparação — qualquer falha de leitura vira uma linha de log.
  - Coberto por `tests/unit/consoleCrashArtifacts.test.cjs`. **O bug continua aberto**: isto não identifica a causa, apenas faz o artefato que falta chegar sem depender de instrução manual.

### Fixed
- **Falhas do catálogo agora mostram a causa e geram diagnóstico (`BrowsePage.tsx`, `browseHandlers.ts`, `backendHttp.ts`) - v2.12.101**:
  - Fecha [o relato do chamado #73](docs/bugs/closed/browse-falha-comunicacao-backend-oculta-erro-sem-telemetria_2026-09-24T21-30.md): a tela preserva o erro retornado pelo backend, incluindo código, endereço/porta ou status HTTP, e permite tentar novamente. Rejeições IPC também saem do carregamento com mensagem; a biblioteca local não transforma falha do catálogo em lista vazia.
  - `browse:get-games` registra `APP_BROWSE` e envia telemetria pelo serviço existente (respeitando a configuração e a deduplicação), com plataforma, fonte, endpoint e stack. Respostas HTTP de erro não viram nomes de jogos; catálogo vazio e progresso de cache continuam válidos.
  - GET e POST do cliente HTTP usam `127.0.0.1`, coerente com o listener IPv4 do Go. Timeouts e respostas interrompidas preservam diagnóstico e encerram a tentativa.
  - Regressões: `tests/unit/backendHttp.test.cjs`, `tests/unit/browseHandlers.test.cjs` e `tests/electron/browse-errors-smoke.cjs`.
- **A atualização in-app nunca chegava à versão nova: o script desistia de sobrescrever o `.exe` e reabria a versão antiga, sem aviso (`services/autoUpdateService.ts`, `ipc/updateHandlers.ts`, `preload.ts`, `app/bootstrap.ts`, `renderer/components/AppUpdateModal.tsx`, `renderer/App.tsx`) - v2.12.100**:
  - Fecha [`docs/bugs/closed/electron-main-atualizacao-in-app-reabre-a-versao-antiga-sem-aviso_2026-09-24T15-18.md`](docs/bugs/closed/electron-main-atualizacao-in-app-reabre-a-versao-antiga-sem-aviso_2026-09-24T15-18.md) (chamado #72; 11 de 11 aplicações observadas na telemetria voltaram na versão antiga).
  - **A espera era por relógio, e o arquivo é do launcher.** No portable o `xboxcompanion.exe` é o launcher NSIS, que só sai depois do Electron fechar **e** de apagar a pasta extraída de ~1,3 GB. O script esperava 1,5 s + 20 × 500 ms a partir do pedido de saída e relançava o alvo mesmo sem ter copiado. Agora ele espera o pid do Electron e todo processo rodando do próprio alvo (até 180 s), e só então copia. Reproduzido em teste: com o "launcher" vivo por ~14 s, o script antigo esgota as 20 tentativas e deixa o `.exe` antigo; o novo troca o arquivo.
  - **Falha deixou de ser silenciosa.** O script grava `update-result.txt`; antes de fechar, o app grava `update-pending.json`. No boot seguinte, se a versão não mudou, o evento vai para a telemetria (hoje esse defeito tinha zero reports) e o modal abre em erro com o motivo, o botão **Mostrar arquivo baixado** e os passos da atualização manual.
  - **O modal passou a ler o `ok` do `update:apply`.** Uma falha antes do `app.quit()` (ex.: `spawn EPERM`, PowerShell bloqueado) deixava o botão em "Reiniciando..." para sempre. O app agora só fecha depois de o PowerShell realmente iniciar.
  - **Alvo inválido é recusado.** Build empacotada sem `PORTABLE_EXECUTABLE_FILE` e com o exe dentro de `%TEMP%` (a cópia extraída pelo portable, aberta por atalho) não é sobrescrita: o modal orienta a atualização manual.
  - **Limite:** a correção vale a partir desta versão. Quem está numa versão anterior ainda roda o script antigo ao aplicar a 2.12.100 e precisa atualizar manualmente uma vez.
- **O app dizia "já existe um desbloqueio" e convidava a pular a preparação sem nunca conferir o perfil do exploit (`services/badAvatarUsbService.ts`, `services/preparedDeviceState.ts`, `services/preparedUsbDetection.ts`, `services/preparedDeviceCopy.ts`, `services/fixedBadAvatarPreparationService.ts`, `infrastructure/exploitProfile.ts`, `infrastructure/preparationReceipt.ts`, `renderer/components/HomePage.tsx`, `BadAvatarUsbPage.tsx`) - v2.12.99**:
  - Fecha [`docs/bugs/closed/badavatar-usb-service-declara-ja-preparado-sem-conferir-o-perfil-do-exploit_2026-09-23T14-08.md`](docs/bugs/closed/badavatar-usb-service-declara-ja-preparado-sem-conferir-o-perfil-do-exploit_2026-09-23T14-08.md), a causa provada de 3 dos 6 clientes de `docs/bugs/open/badavatar-perfil-do-exploit-nao-aparece-na-tela-de-perfis_2026-09-23T15-20.md`.
  - **A pergunta estava errada, não a resposta.** `checkExistingXboxFolders` respondia "este dispositivo tem alguma pasta de Xbox?" com sete indicadores — e **nenhum** deles é exclusivo do modo Bloqueado/LT: `Content/0000000000000000` é criado pelo baixador de jogos sozinho, `Aurora/` e `launch.ini` saem também de um preparo em modo RGH, e `Games`/`FSD`/`Freestyle`/`default.xex` vêm de pacotes de terceiros. O arquivo que decide se o `ABadAvatar` aparece na tela de perfis — o perfil do exploit, `Content/<XUID>/…` — não era conferido em lugar nenhum. Um cliente leu a tela verde em voz alta para o agente e descobriu 40 minutos depois que a preparação nunca tinha rodado naquele pendrive.
  - **O dano era a navegação, não só o texto.** Com o booleano em `true` o assistente abria em `detect`/`prepared-detected`, pulando a pergunta do modo do console, e oferecia "Pular e ir para o catálogo de jogos" como botão primário. Num console travado ou LT, pular garante que a Aurora nunca vai abrir — sem nenhuma mensagem de erro, e a descoberta acontece na frente da TV.
  - **Estado em vez de booleano.** `UsbDriveInfo.preparedState` (`sem-preparo | jogos-apenas | preparado-rgh | preparado-bloqueado-lt`) substitui `alreadyPrepared`. `preparado-bloqueado-lt` exige o perfil do exploit presente e com o tamanho do manifesto; `preparado-rgh` é o marcador `.xbox-downloader/ready-to-play-v<N>.marker` (qualquer versão) ou `Aurora/default.xex` + `launch.ini` sem o perfil; os sete indicadores antigos passaram a distinguir apenas "tem jogos aqui" de "está vazio".
  - **O XUID vem do manifesto, nunca de uma constante.** `exploitProfileFiles()` expõe os caminhos do perfil a partir do manifesto ativo já validado, porque o XUID pertence ao pacote e muda num `npm run payload:update`. A lista é memorizada (a Home pede os dispositivos a cada 5 s, e validar o manifesto relê 643 entradas), e **manifesto ilegível nunca produz "preparado"**: sem poder conferir, o assistente pergunta o modo do console.
  - **Pular a preparação passou a exigir prova.** `canSkipPreparation()` só abre esse caminho para `preparado-bloqueado-lt`, ou para `preparado-rgh` **com o console RGH declarado pelo usuário** — e esse segundo caso é lido no clique de Avançar, não na varredura de 5 s, que trocaria a tela debaixo de quem acabou de escolher o modo do console.
  - **Os textos deixaram de afirmar o que não foi conferido.** Um só módulo guarda o texto por estado: `jogos-apenas` diz que o dispositivo ainda não foi preparado; `preparado-rgh` diz que tem a Aurora para RGH mas **não** tem o perfil do exploit, e manda preparar de novo em *Xbox Bloqueado ou LT* — em alerta âmbar, não em verde de sucesso.
  - **Recibo do preparo (`.xbox-downloader/preparo.json`)**: modo, data, versão do app e release do pacote, que o marcador de uma linha nunca guardou — é o que faltava para o suporte responder **como** um pendrive foi preparado. Gravado **fora** da escrita transacional e depois dela, de propósito: `planHash` sai do conteúdo do plano e o `transactionId` não, então um timestamp dentro do plano faria toda retomada interrompida falhar com "O diário transacional não pertence ao plano de escrita informado".
  - Coberto por `tests/unit/preparedDeviceState.test.cjs`, `preparedDeviceCopy.test.cjs`, `preparationReceipt.test.cjs` e pelo `preparedUsbDetection.test.cjs` reescrito — com falha deliberada comprovada nos dois casos de campo (pendrive só com jogos baixados, e pendrive preparado em RGH) e no texto original.
- **Discos multi-disco No-GOD (como Assassin's Creed IV Disc 2) eram rejeitados quando solicitados como GOD (`src/server/services/pipeline/disc_layout.go`, `disc_layout_test.go`) - v2.12.99**:
  - Fecha [`docs/bugs/closed/backend-pipeline-disco-no-god-rejeitado-quando-pedido-god_2026-09-22T09-08.md`](docs/bugs/closed/backend-pipeline-disco-no-god-rejeitado-quando-pedido-god_2026-09-22T09-08.md) (erro 8181 na telemetria de produção).
  - **Auto-promoção transparente para XEX**: Quando um disco está catalogado na tabela de compatibilidade com `rec.InstallType == "xex"` (ex: Assassin's Creed IV Disc 2, cujo disco multiplayer não é compatível com o formato GOD), o backend verificava `requested != "xex"` e retornava um erro impeditivo (`este disco nao e compativel com GOD; selecione instalacao XEX`), cancelando o download após baixar até 8 GB de dados.
  - A validação agora confere a integridade do executável e promove automaticamente `resolved = "xex"`, idêntico ao comportamento já existente para discos de conteúdo (`content`), gravando no log da aplicação a resolução automática e procedendo com a extração XEX sem interrupção.
  - Coberto pelo teste unitário `TestResolveISOInstallTypePromotesNoGodDiscToXex` em `disc_layout_test.go`.
- **Navegação para abas BadAvatar e Jogos USB forçava reenumeração física e causava timeout no Windows (`renderer/components/BadAvatarUsbPage.tsx`, `UsbGamesPage.tsx`) - v2.12.99**:
  - Complementa a resolução de [`docs/bugs/closed/enumeracao-usb-timeout-e-dispositivo-mudou-desde-a-selecao_2026-09-08T00-00.md`](docs/bugs/closed/enumeracao-usb-timeout-e-dispositivo-mudou-desde-a-selecao_2026-09-08T00-00.md) (62 erros na telemetria, sendo 21 na v2.12.98).
  - As páginas `BadAvatarUsbPage` e `UsbGamesPage` chamavam `toolsBadAvatarListDrives({ fresh: true })` no mount (`useEffect`), descartando o cache de volumes do backend e disparando scripts do PowerShell com sondagem profunda de integridade a cada troca de tela.
  - Alterado para passar `{ fresh: false }` no mount (aproveitando o cache rápido via `stat()`), reservando `{ fresh: true }` estritamente para o clique manual do usuário nos botões "Atualizar" e "Verificar novamente".
- **Build do Electron não checava tipos do renderer e deixava ReferenceError chegar a produção (`package.json`, `src/electron-app/package.json`, `renderer/components/BadAvatarUsbPage.tsx`, `MainNav.tsx`, `UsbGamesPage.tsx`, `tests/unit/rendererBuildTypecheck.test.cjs`) - v2.12.99**:
  - Fecha `docs/bugs/closed/electron-renderer-build-sem-typecheck-deixa-referenceerror-chegar-a-producao_2026-09-17T02-22.md` (9 crashes na telemetria causados por identificadores não declarados no renderer, como `onOpenUpdateModal`).
  - **O processo de build transpilava sem validar tipagem.** `renderer:build` executava `vite build` diretamente, e `tsc` na raiz do app excluía a pasta `renderer/`. Com isso, `test:safety` e todos os comandos de empacotamento (`build:*`) passavam com sucesso mesmo com erros `TS2304` e incompatibilidades de tipos nos componentes React.
  - **6 erros preexistentes de tipagem zerados no renderer**:
    - `BadAvatarUsbPage.tsx`: adicionados campos opcionais preenchidos pelo processo Main (`needsRepair?`, `healthStatus?`, `operationalStatus?`) à interface `UsbDrive`.
    - `MainNav.tsx`: removida comparação morta `currentPage === "home"` em bloco condicional `{!onHome}` e simplificada atribuição para `variant="default"`.
    - `UsbGamesPage.tsx`: ajustado `variant="default"` no botão de confirmação de exclusão (estilização preservada via classes Tailwind).
  - **Checagem de tipos obrigatória em testes e empacotamento**: criado o script `renderer:typecheck` (`tsc --noEmit -p renderer/tsconfig.json`), integrado como pré-requisito antes de `vite build` em `renderer:build` e como etapa de validação em `test:safety`.
  - Coberto por `tests/unit/rendererBuildTypecheck.test.cjs` e validação de aceitação (falha deliberada comprovada em tempo de build/teste caso haja identificador não declarado).
- **App parado na Home abria PowerShell sem parar e piscava janelas de terminal (`services/badAvatarUsbService.ts`, `ipc/badAvatarHandlers.ts`, `ipc/driveMaintenanceHandlers.ts`, `preload.ts`, `renderer/components/HomePage.tsx`, `BadAvatarUsbPage.tsx`, `UsbGamesPage.tsx`) - v2.12.98**:
  - A Home consulta o pendrive a cada 5 s e o contador de jogos instalados a cada 7 s, inclusive com a janela em segundo plano. No Windows cada consulta de `listFat32UsbDrives()` rodava a enumeração completa: dois ou três `powershell.exe`, mais `csc.exe`/`cvtres.exe` do `Add-Type` e `mountvol.exe`, sem deduplicar chamadas simultâneas (foram medidos quatro PowerShells no mesmo segundo). Em algumas máquinas cada um desses processos pisca uma janela de console, então o app ocioso enchia a tela de terminais.
  - **A lista agora é reaproveitada até um volume montado mudar.** A assinatura é a letra e o número de série de cada raiz, lidos com `stat()` (`dev` é o serial do volume; 26 letras em ~4 ms, sem abrir processo). Formatar gera serial novo e ejetar tira a letra, então os dois reenumeram sozinhos. Só letra inexistente (`ENOENT`) sai da assinatura: um pendrive RAW (novo, ext4, FAT corrompida) tem letra mas `stat` falha, e continua contando — senão ele nunca apareceria para ser formatado. Chamadas simultâneas dividem uma única enumeração, mas quem chega depois de uma invalidação não pega carona na enumeração anterior. O relógio do assentamento é monotônico (`performance.now()`), para um ajuste de hora para trás não religar o polling.
  - **Ação do usuário continua reenumerando.** `tools:badavatar-list-drives` aceita `{ fresh: true }`, enviado só por "Atualizar" (BadAvatar e Jogos no USB, inclusive depois de preparar, ejetar, excluir e reparar) e pela verificação manual da Home. Trava de gravação desligada por software ou uma sonda de integridade que estourou o tempo não mudam letra nem serial, e a mensagem de bloqueio manda justamente clicar em Atualizar. Os polls de 5 s e 7 s continuam no cache.
  - **Volume recém-mudado continua sendo reenumerado por 15 s.** O Windows mostra a letra alguns segundos antes de o Storage Management listar o disco (no log de eventos, a chegada do disco antecede a detecção em ~4 s); guardar essa primeira resposta vazia esconderia o pendrive até reconectar.
  - **Espaço livre e "já preparado" continuam frescos.** `freeBytes` é relido por `statfs` a cada chamada (bate byte a byte com `Get-Volume`), porque downloads para o pendrive não mudam o serial, e `alreadyPrepared` já era recalculado por chamada. O reparo (chkdsk) não muda o serial, então o handler `tools:drive-repair` invalida a lista para o aviso de reparo sumir.
  - A segurança da gravação não muda: preparação, prévia e a revalidação de 10 s continuam chamando `enumerateSafeWindowsUsbDevices` sem cache e conferindo a impressão digital do dispositivo.
  - Coberto por `tests/unit/usbDriveListCache.test.cjs`: falha no código anterior (5 enumerações onde se esperam 3), e também se o volume RAW sair da assinatura ou se uma chamada pós-invalidação reaproveitar a enumeração antiga. Medido no app: 90 s parado na Home, 18 pedidos da lista e nenhum `powershell.exe` (antes, 34 em 100 s).
- **Discos de conteúdo sem executável na raiz da ISO eram recusados antes da checagem de compatibilidade (`src/server/utils/iso2god.go`, `services/pipeline/disc_layout.go`, `digital.go`, `models/compat.go`, `infrastructure/helpers/multi_disc.go`, `interfaces/http/handlers.go`) - v2.12.97**:
  - Fecha `docs/bugs/closed/backend-pipeline-disco-de-conteudo-sem-default-xex-recusado_2026-09-15T11-40.md` (erros 6699 e 6726 na telemetria, falha com `validar tipo do disco: no game executable (default.xex / default.xbe) found in ISO root`).
  - **Diferenciação entre falta de executável e corrupção (`utils/iso2god.go::ErrNoExecutable`)**: `extractExecInfo` agora retorna o erro sentinela `ErrNoExecutable` quando não há `default.xex`/`default.xbe` na raiz. Erros de partição XGD ou descritor de volume XDVDFS continuam sendo tratados como corrupção estrutural da imagem.
  - **Resolução de instalação de conteúdo sem executável (`services/pipeline/disc_layout.go`)**: `resolveISOInstallType` trata `ErrNoExecutable` com `info` inicial vazio e permite que `ProbeISOInstallLayout` + `GuessTitleIDFromMultiDiscName` + `DiscNumberFromName` decidam. Se a tabela de compatibilidade ou o layout do disco indicarem `content`, a ISO é aceita como disco de conteúdo. A recusa por falta de executável é mantida caso o disco não traga conteúdo instalável nem registro de compatibilidade como `content`, e pedidos explícitos de `xex` são bloqueados.
  - **Suporte end-to-end e resiliência a ponteiro nulo (`services/pipeline/digital.go`, `utils/iso2god.go`)**: `processContentInstallFromISO` aceita `ErrNoExecutable`, derivando Title ID do conteúdo ou do nome da mídia e populando `info.DiscNumber`/`info.DiscCount`. `ProbeISOInstallLayout`, `ExtractXDVDFSContentToDir` e `ProbeContentPackageTitleID` foram tornados seguros contra `info == nil` e `info.TitleID == 0`.
  - **Reconhecimento de `(Disco N)` em Português/Espanhol (`models/compat.go`, `infrastructure/helpers/multi_disc.go`)**: `discTagPattern`, `discTrailingPattern`, `discSubtitlePattern` e `genericDiscFolderName` agora aceitam `(Disco N)` e `(Disco N de M)`. Títulos como `Batman - Arkham Origins (Brazil) (En,Es,Pt) (Disco 2)` agora resolvem `DiscNumber = 2`, agrupam-se com o Disco 1 e casam com a tabela de compatibilidade.
  - Coberto por testes automatizados em `compat_test.go`, `god_validate_test.go` e `disc_layout_test.go`.
- **Os cinco pontos restantes de extracao nunca invalidavam o arquivo corrompido, e a telemetria titulava o defeito com a mensagem errada (`services/pipeline/rom.go`, `minerva.go`, `fallback.go`) - v2.12.97**:
  - Fecha `docs/bugs/closed/backend-pipeline-arquivo-corrompido-reaproveitado-do-cache_2026-09-12T12-24.md`. A 2.12.94 tinha ligado `invalidateDownloadedArchiveOnCorruptExtract` em HuggingFace, IA generico, IA digital e Redump/ISO, mas cinco chamadas de `extractArchiveResilient` ficaram de fora — e nelas o mesmo arquivo ruim continuava sendo reaproveitado a cada tentativa.
  - **ROM/EdgeEmu tinha o defeito identico ao do bug original.** Em modo local `zipPath` e `Ready/<jogo>/.source_rom.zip`, baixado por `DownloadWithProgress` (que grava o marcador de conclusao com o SHA-256 do proprio arquivo) e retido sem `defer os.Remove`. Um `zip: checksum error` devolvia o erro direto, entao o ZIP corrompido ficava no cache com hash que batia consigo mesmo.
  - **Minerva e pior que o cache de HTTP: reaproveita por tamanho.** `DownloadViaTorrent` aceita `destDir/<arquivo>` quando so o `os.Stat` bate com `fileSize` (`torrent.go:396`) — nem hash existe —, e em modo local `cleanupTorrentScratchAfterRun` preserva o `torrentDir` enquanto o job nao chega a `Ready`. Os quatro pontos de extracao do Minerva (XEX, ISO, generico, digital) agora invalidam o arquivo em erro de integridade, o que basta para o proximo `os.Stat` falhar e o aria2c rebaixar.
  - **O titulo da telemetria escondia a causa em vez de mostra-la.** O reporte usava `lastErr`, que e quase sempre o ultimo provedor da cadeia dizendo `jogo nao encontrado no catalogo`. Foi assim que quatro ocorrencias de `rardecode: bad file checksum` no HuggingFace foram arquivadas sob a mensagem do Minerva, agrupadas pela mensagem errada. O titulo passa a citar o primeiro erro que **nao** e ausencia de catalogo, com o nome do provedor; `providerErrors` continua indo integral nos logs.
  - Coberto por `TestExtractionErrorClassificationsDoNotOverlap`, que trava a invariante de que a correcao depende: nenhuma mensagem de integridade pode ser lida como falha de armazenamento (viraria halt de dispositivo, preservando o arquivo ruim para sempre) e nenhuma mensagem de armazenamento pode ser lida como integridade (apagaria um download integro de varios GB por causa de disco cheio).
- **Download em modo local ignorava o volume mais folgado e estourava a partição do sistema (`src/server/app/app.go`, `config.go`, `services/pipeline/workspace.go`, `huggingface.go`, `pipeline.go`, `digital.go`, `rom.go`, `minerva.go`, `content.go`, `services/local/scanner.go`, `interfaces/http/handlers.go`) - v2.12.96**:
  - Fecha `docs/bugs/closed/2026-09-06-fallback-abortado-por-erro-classificado-como-dispositivo-local.md` (item 7, o último item em aberto do incidente de classificação de dispositivo local).
  - **Alocação de staging no volume com mais espaço livre (`App.ReadyDir` / `GetReadyDir`)**: em modo local (USB/pendrive), o `archivePath` do download de arquivos grandes (.source* de 5 a 10 GB) era derivado de `ToolsDir/Ready/<jogo>`, fixo no drive do aplicativo (`C:`). Quando `C:` tinha pouco espaço livre, o download falhava com falso erro de espaço insuficiente no armazenamento temporário, mesmo com centenas de GBs livres em outro volume fixo (`D:`). O diretório `ReadyDir` agora acompanha a auto-seleção de `bestFixedVolume()` e passa a residir em `godsend-temp/ready` na partição mais folgada.
  - **Preservação de retenção e resiliência local**: a retenção de downloads concluídos fora do scratch transiente (`cleanupGameScratch`), a sobrevivência do arquivo baixado íntegro em caso de falha de escrita no USB (`State == "Error"`), a limpeza pontual nos 28 locais com `os.RemoveAll(gameDir)` e a limpeza de diretórios vazios (`CleanupEmptyReadyDirs`) permanecem totalmente garantidas.
  - **Retrocompatibilidade e tolerância a falhas**: `resolveLocalSourceArchive()` reutiliza downloads completos pré-existentes na pasta legada `ToolsDir/Ready`; `EnsureWorkingVolume()` reverte `ReadyDir` para `ToolsDir/Ready` se o volume secundário for desconectado durante a sessão; `handleFileServe` serve arquivos com fallback para a pasta legada.
  - Coberto por novos testes automatizados em `scratch_test.go` (`TestGetReadyDirDefaultsAndExplicit`, `TestEnsureWorkingVolumeRevertsReadyDir`) e `local_resilient_test.go` (`TestResolveLocalSourceArchivePrefersActiveReadyDir`).
- **Volume de trabalho auto-selecionado sumia e derrubava o backend ou todos os jobs da sessão (`src/server/app/staging_windows.go`, `staging_other.go`, `config.go`, `app.go`, `services/pipeline/fallback.go`, `pipeline.go`, `rom.go`) - v2.12.95**:
  - Fecha `docs/bugs/closed/backend-volume-temp-autoselecionado-indisponivel-derruba-backend_2026-09-13T15-59.md` (11 reports de produção, crash de boot com `os.Exit(1)` e 10 jobs falhando em sequência).
  - **Exclusão de barramentos USB na auto-seleção de volume (`staging_windows.go`, `staging_other.go`)**: `isExternalOrUSBBus()` consulta `IOCTL_STORAGE_QUERY_PROPERTY` para `STORAGE_DEVICE_DESCRIPTOR` (verificando `BusTypeUsb`, `BusType1394`, `BusTypeSd`, `BusTypeMmc` e `removable`). O Windows reporta HDs/SSDs externos USB como `DRIVE_FIXED`, o que burlava o filtro anterior; `bestFixedVolume()` agora ignora estritamente qualquer volume em barramento USB, evitando que discos externos que podem dormir ou ser desconectados sejam escolhidos como scratch.
  - **Tolerância a falhas no boot (`app/config.go::SetupPaths`)**: se um volume auto-selecionado falhar na marcação de dono (`markScratchOwner`, ex.: disco removido ou com `The device is not ready`), `SetupPaths` não aborta com erro fatal: registra aviso e reverte imediatamente `TempDir` e `TorrentTempDir` para `ToolsDir/Temp`. Apenas falhas no próprio disco do app (`ToolsDir`) ou em caminhos explicitamente definidos pelo usuário (`GODSEND_TORRENT_TEMP`) são fatais.
  - **Revalidação de volume de trabalho por job (`app/app.go::EnsureWorkingVolume`, `AcquireGameJob`, `services/pipeline/`)**: se um volume temporário for desconectado durante a sessão, `EnsureWorkingVolume()` testa a escrita no início de cada job (`AcquireGameJob`, `ProcessGameWithFallback`, etc.) e reverte para `ToolsDir/Temp` com aviso, impedindo que a queda de um disco derrube todos os jobs subsequentes da fila.
  - Coberto por testes automatizados em `staging_windows_test.go` e `scratch_test.go`.
- **Múltiplas instâncias do aplicativo abriam simultaneamente e conflitavam por USB, fila, scratch e binários (`src/electron-app/main.ts`, `app/bootstrap.ts`, `app/window.ts`, `package.json`, `src/server/app/homelock.go`, `homelock_windows.go`, `homelock_other.go`, `config.go`, `queue_resume.go`, `main.go`) - v2.12.95**:
  - Fecha `docs/bugs/closed/electron-main-multiplas-instancias-sem-single-instance-lock_2026-09-15T14-44.md` (184 erros reportados na telemetria, pico de até 17 instâncias simultâneas na mesma máquina). O usuário fechava no X (indo para a bandeja) e abria o executável de novo, ou clicava várias vezes no executável portátil durante a descompactação silenciosa, gerando múltiplos processos concorrentes.
  - **Trava de instância única no Electron (`main.ts`, `bootstrap.ts`, `window.ts`)**: `main.ts` agora solicita `app.requestSingleInstanceLock()` imediatamente após fixar `userData`. Se outra instância já estiver rodando, o processo encerra imediatamente via `app.quit()`. Na instância primária, o evento `second-instance` invoca `focusMainWindow()`, que restaura a janela caso minimizada, desoculta-a caso oculta na bandeja (`close-to-tray`) e dá foco no primeiro plano.
  - **Trava exclusiva sobre `GODSEND_HOME` no backend Go (`app/homelock.go`, `homelock_windows.go`, `homelock_other.go`)**: para proteger contra execuções manuais ou instâncias antigas sem trava, `*App.AcquireHomeLock()` cria e trava exclusivamente `.godsend.lock` via `LockFileEx` no Windows (offset 4096 com `LOCKFILE_EXCLUSIVE_LOCK | LOCKFILE_FAIL_IMMEDIATELY`) e `syscall.Flock` no POSIX. O kernel do SO libera a trava automaticamente mesmo em crash. `SetupPaths()` recusa inicialização se o diretório já estiver travado, informando o PID ativo; `ResumeQueuedJobs()` e o restore de FTP checam `HasHomeLock()`.
  - **Posse de scratch não é mais roubada de processo ativo (`app/config.go::markScratchOwner`)**: `markScratchOwner` confere se `.godsend-owner.pid` existe e aponta para um processo ainda vivo (`processIsRunning(pid)` e `pid != os.Getpid()`). Se estiver vivo, a posse não é sobrescrita, impedindo que a limpeza stale apague os arquivos de trabalho de um backend ativo.
  - **Isolamento de extração no portátil (`src/electron-app/package.json`)**: alterado `unpackDirName` para `true` no alvo `portable`. Evita o defeito do electron-builder onde `false` definia uma pasta com ksuid fixo em `%TEMP%`, garantindo que cada execução descompacte em `$PLUGINSDIR\app` isolado e que uma instância ao fechar ou abrir não apague binários (`aria2c.exe`) de outra.
  - Coberto por `tests/unit/singleInstance.test.cjs` no Electron e `app/homelock_test.go` no backend Go.
- **Falha ao criar a pasta de logs derrubava o app e impedia o backend de subir (`infrastructure/serverLog.ts`, `app/bootstrap.ts`) - v2.12.95**:
  - Fecha `docs/bugs/closed/electron-main-falha-na-pasta-de-logs-impede-backend-de-subir_2026-09-13T15-59.md` (errors 6274/6275 e 6642/6643, duas maquinas em dois dias). O usuario via o dialogo "GODsend crashed" e o backend **nunca era iniciado** - porque nao deu para escrever uma linha de log.
  - **`getLogInfo()` fazia I/O.** Ele chamava `ensureLogDir()`, que e um `fs.mkdirSync` puro, e deixava a excecao subir. As funcoes de gravacao (`appendLine`, `appendBackendSessionStart`, `appendBackendSessionEnd`, `openLogsFolder`) sempre protegeram o `mkdir` com `try/catch`; so o getter de caminhos nao protegia. Agora `getLogInfo()` e um getter puro: devolve `logsDirectory` e `currentLogFile` sem tocar no disco, e a criacao da pasta fica com quem grava - que ja tolera a falha.
  - **Por isso uma pasta opcional parava o app.** `startGodsend()` chamava `getLogInfo().logsDirectory` so para imprimir a linha `[INFO] Server logs: ...`, **antes** do `spawn` do backend; como ele roda dentro de `webContents.once("did-finish-load")`, a excecao virava `uncaughtException`. O mesmo `getLogInfo()` ainda aparecia no `app ready` (`bootstrap.ts`), no handler `logs:get-info` (que repassava o erro ao renderer, virando um `unhandledrejection` em `main.tsx`) e dentro do proprio handler de crash, que tentava citar o arquivo de log no dialogo e lancava de novo.
  - **A causa ambiental continua sendo ambiental.** `mkdirSync(..., { recursive: true })` so da `ENOENT` numa pasta ja existente se um componente do caminho sumiu naquele instante (`Downloads` redirecionado para um volume que caiu, sincronizacao em nuvem, limpeza de terceiros) - e o mesmo processo tinha chamado `getLogInfo()` com sucesso segundos antes. Nao da para impedir que o volume caia; da para nao derrubar o aplicativo por causa disso.
  - **Subir o backend passou a ter um unico ponto de entrada protegido** (`services/backendClient.ts`). `startGodsendSafely()` registra a excecao no log, reporta a telemetria e mostra `[ERROR] Could not start the backend: ...` no painel de saida, em vez de matar o processo principal com o backend parado. `startGodsend()` deixou de ser exportado, entao nao ha como chamar a versao desprotegida de fora.
  - **O `did-finish-load` nao era o unico chamador desprotegido.** `restartGodsendIfRunning()` chamava `startGodsend()` dentro de um `setTimeout` - e excecao em callback de timer tambem e `uncaughtException` fatal -, e as 16 opcoes de configuracao que reiniciam o backend (`ipc/configHandlers.ts`) passam por ali. Os handlers IPC `godsend:start` e `godsend:restart` tinham o mesmo furo, com o erro virando `unhandledrejection` no renderer. Os quatro agora passam pelo guard.
  - Coberto por `tests/unit/serverLogResilience.test.cjs`: com `fs.mkdirSync` lancando `ENOENT`, `getLogInfo()` devolve os caminhos, nenhuma funcao de gravacao lanca e `openLogsFolder()` reporta o erro no retorno.
- **Arquivo baixado corrompido ficava preso no cache e falhava igual em toda tentativa (`infrastructure/download/ia.go`, `services/pipeline/archive_cache.go`, `huggingface.go`, `digital.go`, `pipeline.go`) - v2.12.94**:
  - Fecha `docs/bugs/closed/backend-pipeline-arquivo-corrompido-reaproveitado-do-cache_2026-09-12T12-24.md`. Quando a extracao prova corrupcao de integridade (`rardecode: bad file checksum`, CRC/ZIP/7z invalido, arquivo extraido incompleto), o pipeline agora remove o arquivo retido, o marcador `.xbox-companion-complete.json` e o checkpoint de resume. A proxima tentativa baixa de novo em vez de reutilizar bytes ruins que tinham hash de si mesmos.
  - A limpeza e deliberadamente estreita: erro de disco cheio, permissao, I/O local ou destino desconectado continua preservando o download, porque outro download nao corrige armazenamento local.
  - O mesmo mecanismo vale para HuggingFace, IA generico, IA digital e Redump/XEX/ISO em modo local, todos os pontos que preservam `.source*` fora do scratch transiente.
  - Um marcador `.corrupt-redownloaded` limita a uma rebaixada: se o arquivo baixado novamente falhar igual, a mensagem passa a apontar origem possivelmente corrompida no catalogo, e o arquivo invalido tambem e removido para nao virar loop de cache.
  - Coberto por testes que garantem a remocao de arquivo + markers em erro de integridade e a preservacao do download em erro de armazenamento.
- **Disco cheio do PC era relatado como "Falha no dispositivo local", e so parava a fila em modo pendrive (`services/pipeline/local_resilient.go`, `stage_checkpoint.go`, `fallback.go`) - v2.12.93**:
  - Fecha os itens 3, 6 e 8 de `docs/bugs/closed/2026-09-06-fallback-abortado-por-erro-classificado-como-dispositivo-local.md`. Os itens 1, 2, 4 e 5 ja tinham sido fechados na 2.12.73; o item 7 continua aberto, agora com medicao nova (ver abaixo), e por isso o arquivo fica em `open/` (movido para `closed/` na 2.12.96, quando o item 7 fechou).
  - **Extracao e conversao GOD tambem rodam no disco do PC, e tambem diziam "dispositivo local".** `outputRoot()` devolve `s.App.TempDir` incondicionalmente, e foi confirmado que **todo** `godDir` e `extDir` do projeto sai dali (`pipeline.go:137,297`, `digital.go:254`, `minerva.go:155,200`) - a conversao GOD, o passo que mais consome disco, incluida. So a fase `download-*` era marcada como falha do PC; nas outras o usuario era mandado conferir o pendrive por causa de um disco cheio em `C:`. A classificacao agora cobre `download-*`, `extract-*` e `convert-*`.
  - **Em modo FTP o mesmo disco cheio nao parava nada.** A classificacao exigia `Mode == "local"` logo na primeira linha, entao em modo FTP o erro caia no `recordError` e a cadeia seguia para `ia` e `minerva` - enchendo o **mesmo** disco mais duas vezes antes de falhar igual. Nenhum dos dois comportamentos tinha sido decidido de proposito. O volume de trabalho e o mesmo nos dois modos, entao a cadeia agora para em todos eles.
  - **E a mensagem parou de culpar hardware que nao existe.** `ErrLocalStaging` nao vem mais embrulhado em `ErrLocalDelivery`: em modo FTP nao ha dispositivo local nenhum, e "falha no dispositivo local" era simplesmente falso. Quem decide a parada e `isLocalStorageHalt`, que aceita as duas sentinelas - o disco do PC e o dispositivo de destino continuam parando a fila, cada um com o seu texto.
  - **`%v` deixou de achatar o erro original.** As duas construcoes de `classifyLocalStorageFailure` e as duas de `runDirectoryStage` passaram para `%w`. Com `%v`, qualquer sentinela abaixo da classificacao ficava invisivel para `errors.Is` - hoje `ErrJobCancelled` nao casa nenhum fragmento de armazenamento, mas bastaria uma mensagem carregar "acesso negado" para um cancelamento do usuario ser relatado como disco cheio, e `fallback.go` decide pelo `errors.Is`, nao pelo texto.
  - **`os.MkdirAll` com disco cheio e a falha identica vinda de `action()` seguiam caminhos diferentes.** `stage_checkpoint.go` construia `ErrLocalDelivery` a mao em dois pontos, sem passar pelo classificador - entao o filtro de rede da 2.12.73 tambem nao valia ali. Os dois pontos agora chamam `classifyLocalStorageFailure`, e a ramificacao redundante que so existia para o modo local saiu.
  - Coberto por quatro testes: a simetria entre modos, as quatro fases que rodam no disco do PC, a sobrevivencia das sentinelas ao `errors.Is`/`errors.As`, e a fase de extracao apontando o disco certo. `TestIsNetworkErrorSeparatesDiskFromSocket` continua travando a armadilha do filtro de rede.
  - **Item 7 (`archivePath` do modo local ignora o volume mais folgado) segue aberto**, com a estimativa corrigida no arquivo do bug: nao sao os 2 pontos de troca de caminho que o relatorio original contava, e sim 5 (`huggingface.go`, `pipeline.go`, `digital.go` x2, `rom.go`), e tirar o arquivo de `ToolsDir/Ready` exige mexer nos 28 `os.RemoveAll(gameDir)` que hoje o limpam - `ProcessHuggingFaceGame` e `ProcessDigital` nao passam por `cleanupCompletedLocalScratch`. Alem disso `protectedScratchPaths` nao protege job em estado `Error`, que e exatamente o estado em que esta falha deixa a fila: mover o arquivo para `TempDir` faria o download de varios GB ser apagado no proximo boot do aplicativo.
- **O hook do Aurora podia reiniciar o console a cada boot do pendrive preparado (`infrastructure/readyToPlayConfiguration.ts`) - v2.12.92**:
  - Trabalha o candidato 2 de `docs/bugs/closed/2026-09-05-fatal-crash-intercepted-pendrive-preparado.md`. O `Aurora.Restart()` do `XboxCompanionReady.lua` e deliberado - os scan paths so valem na inicializacao seguinte -, mas ele era disparado por `changed`, e nada garantia que `changed` fosse falso no boot seguinte. Um cadastro que se repetisse virava reinicio a cada boot: para o usuario, o console reiniciando ou travando logo ao inserir o pendrive.
  - **Uma divergencia estava documentada no proprio pacote.** Em `Aurora/Data/Logs/debug.log`, o mesmo dispositivo e montado como `\??\Usb0:` com o serial fisico e como `\??\Game:` com esse serial prefixado por `_`. O script ja tinha `normalizedSerial` e o usava para correlacionar o mount `Game:`, mas comparava `deviceid` por igualdade crua em `ensureScanPath` - inconsistencia dentro do mesmo arquivo, que fazia a linha ja gravada parecer de outro aparelho. A comparacao agora e normalizada.
  - **O laco deixou de ser possivel, nao so improvavel.** Antes de reiniciar, o hook grava `.xbox-downloader/ready-to-play-v4.restart` no pendrive e confere a gravacao relendo o arquivo; se o cadastro se repetir no boot seguinte - por essa causa ou por outra ainda desconhecida -, a marca ja existe e ele desiste do reinicio, registrando `caminhos recadastrados sem reiniciar` no `debug.log`. Um boot que encontra o banco correto apaga a marca, devolvendo o direito a um reinicio caso o Aurora recrie o banco. O pior caso passa a ser "os jogos podem nao aparecer sem um reinicio manual", com rastro no log.
  - Nada disso prova que o laco causava o banner **Fatal Crash Intercepted!**: o bug continua aberto e ainda depende de um dump de reproducao real. O que mudou e que o `debug.log` agora separa "a divergencia existe" de "o console esta reiniciando sozinho".
  - **`READY_TO_PLAY_CONFIGURATION_VERSION` passa de `3` para `4`**, pelo mesmo motivo da 2.12.68: os bytes gerados mudaram e `verifyTransactionJournalAgainstPlan` recusaria o diario antigo do pendrive sob o mesmo `transactionId`. O marcador gravado passa a ser `.xbox-downloader/ready-to-play-v4.marker`; o `v3` de uma preparacao anterior fica no dispositivo sem uso.
- **Fallback e Catalogo Online entregavam a release de outro pais, e juntavam jogos diferentes numa entrada so (`services/cache/matching.go`, `interfaces/http/handlers.go`) - v2.12.91**:
  - Fecha o que restava no codigo do bug `docs/bugs/closed/2026-08-09-fallback-huggingface-archive-minerva.md`. A cadeia `huggingface -> ia -> minerva` ja resolvia todo titulo publicado (sonda sobre os caches empacotados: nenhum titulo sem provedor, nenhum provedor que tem o jogo e nao resolve o nome disparado), mas o casamento de variantes introduzido la tinha defeitos proprios.
  - **Pedido sem regiao caia na release de um pais so.** O nome que o HuggingFace publica nao tem regiao ("Red Dead Redemption"); quando o HF falhava, IA e Minerva escolhiam a variante com **menos tags** - `(Japan)`, `(Russia)`, `(Germany)` - com `(USA, Europe) (En,...)` no mesmo catalogo. Medido: 226 titulos no IA e 269 no Minerva. Agora um pedido que nao fixa regiao nem idioma prefere release em ingles ou portugues - pela lista de idiomas quando ela existe (`(Europe) (It,Pl,Ru)` nao conta como ingles), pela regiao so quando nao ha lista - sem trocar de disco. Pedido que nomeia a regiao continua com ela.
  - **Demo, beta e promo nao ganham mais do jogo retail** quando o pedido nao os pede. Continuam alcancaveis quando sao a unica variante.
  - **O Catalogo Online listava a primeira variante em ordem alfabetica**, e `(Japan)`/`(Russia)` vem antes de `(USA)`. Como o nome listado e o que o `/trigger` recebe, o jogo era listado e baixado na versao de outro pais (115 titulos). Agora, dentro do provedor dono do titulo, fica a variante que o fallback baixaria; o dono continua sendo o primeiro provedor da prioridade.
  - **Titulo com ponto era cortado no ultimo ponto.** `filepath.Ext` tratava `. Raw 2011 (USA, Europe)` como extensao, e `WWE SmackDown vs. Raw 2007...2011` viravam uma chave so - assim como `F.E.A.R. 2`/`3`, os quatro `MX vs. ATV`, `Plants vs. Zombies` e `Garden Warfare`, `L.A. Noire` e a `Complete Edition`: 19 chaves fundindo jogos diferentes. O catalogo mostrava um e escondia os outros, e o fallback pontuava todos como igualdade exata e podia baixar outro jogo. Agora so `.zip`, `.7z`, `.rar` e `.iso` sao removidos.
  - Medicao final sobre os caches empacotados:
    - pedido sem regiao caindo em release de outro pais com a release em ingles no mesmo provedor: 226 -> 0 no IA e 269 -> 0 no Minerva;
    - listagem na variante errada dentro do provedor: 115 -> 0;
    - chaves que fundiam jogos diferentes: 19 -> 0, e o Catalogo Online passou de 2964 para 2986 titulos (22 jogos que estavam escondidos);
    - os 9 jogos do print resolvem em todos os provedores que os tem, e `Aliens vs Predator`, que nao casava no HuggingFace, agora casa.
  - Regressoes em `matching_test.go` e `browse_unified_test.go`. O teste do browse foi verificado por mutacao: sem a guarda de provedor no merge, ele falha.
- **Midia de demo era baixada e instalada junto com o jogo, sem ninguem pedir (`models/compat.go`, `compat_test.go`) - v2.12.90**:
  - `discSubtitlePattern` casa uma palavra-chave de papel de disco em **qualquer lugar** do grupo entre parenteses. `(Multiplayer Demo)` contem `Multiplayer`, entao era removido como se fosse papel de disco: `NBA 2K (USA) (Multiplayer Demo)` virava `NBA 2K (USA)`, o mesmo titulo de release da linha retail. Dai em diante o demo era um irmao legitimo para o `FindCompanionDiscs`, e o `enqueueCompanions` - que nao tem filtro nenhum - baixava e instalava a midia junto com o jogo.
  - Medido antes da correcao: `FindCompanionDiscs("NBA 2K (USA)", [retail, demo])` devolvia **as duas linhas**. Depois, devolve so a retail.
  - **Correcao:** grupo que nomeia midia nao-retail (`demo`, `beta`, `trial`, `preview`) deixa de ser tratado como papel de disco e **sobrevive no titulo**, que e o que distingue as duas linhas. `beta`/`trial`/`preview` entraram junto porque sao o mesmo defeito, nao escopo novo: `(Multiplayer Beta)` falhava exatamente como `(Multiplayer Demo)`.
  - Um `(Demo)` sozinho **nunca teve o bug** e continua igual: sem palavra-chave de papel, o grupo ja sobrevivia ao titulo. So a combinacao com papel de disco quebrava.
  - **O demo nao sumiu do catalogo.** Ele continua navegavel e baixavel por si; o que mudou e que parou de entrar de carona na release e de contar para o conjunto de discos.
  - Coberto por dois testes que se equilibram: um prova que a midia nao-retail nao vira disco companion, o outro prova que o corte de papel de disco de que as releases multi-disco reais dependem continua funcionando (`(Disc 1) (Install)` e `(Disc 2) (Play)` do GTA V ainda colapsam no mesmo titulo, e `(Triple Pack)` continua sobrevivendo).
- **O Defender barrava a formatacao antes de ela comecar: "Alteracoes nao autorizadas bloqueadas ... impediu \Device...\fat32format.exe de fazer alteracoes na memoria" (`fat32Format.ts`, `fat32FormatGuard.test.cjs`) - v2.12.89**:
  - Print de um cliente, na tela da Seguranca do Windows. **Nao e o mesmo bloqueio da 2.12.88**: ali a causa era nossa (a pre-formatacao NTFS entregue ao `fat32format`), aqui e o **acesso controlado a pastas** - a protecao contra ransomware do Defender, que barra aplicativo nao confiavel de escrever setores direto num volume. O `fat32format.exe` e binario de terceiro sem assinatura conhecida do Defender e faz exatamente isso. O sintoma que chega ao nosso codigo e o mesmo `GetLastError()=5`, e por isso o conserto da 2.12.88 sozinho nao destravaria a maquina deste cliente.
  - **A formatacao passou a se desbloquear sozinha.** Antes de formatar, o script elevado le `(Get-MpPreference).EnableControlledFolderAccess`; nos modos que realmente bloqueiam (1 = ligado, 3 = so bloqueio de disco; 2 e 4 sao auditoria e deixam a escrita passar) ele poe o caminho do `fat32format.exe` em `ControlledFolderAccessAllowedApplications` e **retira ao final**. O cliente nao precisa achar a tela da Seguranca do Windows nem saber o que e acesso controlado a pastas.
  - **A liberacao e temporaria de verdade, inclusive quando a formatacao falha.** A retirada fica num `finally` porque o `catch` do script faz `exit 1`; foi verificado nesta maquina que o PowerShell roda o `finally` antes de efetivar esse `exit` (script de prova: imprimiu `FINALLY RODOU` e preservou o codigo de saida 7). Sem isso, uma formatacao malsucedida deixaria um executavel de terceiro permanentemente liberado na protecao contra ransomware do usuario - o tipo de coisa que ninguem lembra de desfazer.
  - **Nada e desligado globalmente, e uma sobra de execucao anterior e adotada em vez de virar permanente.** `Set-MpPreference` nao e chamado em lugar nenhum: a protecao continua ligada e so o nosso executavel e excecionado. O flag `$script:cfaAllowlistAdded` vive so no processo elevado, entao uma execucao morta antes do `finally` (queda de energia, processo derrubado) deixaria a entrada na lista - e, pior, toda execucao seguinte a veria como preexistente e nunca a retiraria, exatamente o cenario que esta correcao existe para evitar. Por isso, encontrar o nosso `fat32format.exe` ja na lista faz o script **adotar** a entrada e retira-la no fim: o estado final e sempre “fora da lista”. A contrapartida assumida: quem tiver liberado esse mesmo caminho a mao perde a liberacao - e nao precisa mais dela, porque agora e automatica.
  - **Defender gerenciado por politica deixou de ser beco sem saida.** Quando o `Add-MpPreference` falha (tipico de maquina com GPO/Intune), a mensagem de acesso negado passa a nomear o acesso controlado a pastas e a ditar o caminho - Seguranca do Windows > Protecao contra virus e ameacas > Protecao contra ransomware > Permitir um aplicativo - ja com o caminho do `fat32format.exe` daquela instalacao. O log de diagnostico tambem passou a registrar o modo do acesso controlado, ao lado do estado do disco e da politica de escrita em removiveis que a 2.12.88 acrescentou.
  - **Limite do que foi verificado:** as funcoes geradas foram executadas contra o Defender desta maquina e o caminho de no-op esta provado (modo 0 detectado, nenhuma alteracao feita, nenhum ruido no log), assim como nomes e tipos dos parametros (`Add-MpPreference`/`Remove-MpPreference` aceitam `ControlledFolderAccessAllowedApplications` como `String[]`; `EnableControlledFolderAccess` e `Byte`, dai o cast `[int]`). O ciclo completo de adicionar e retirar **com o acesso controlado ligado** nao foi reproduzido: ele esta desligado aqui e liga-lo mudaria a postura de seguranca da maquina. Ler a lista de permitidos exige elevacao - sem admin o `Get-MpPreference` devolve o texto `N/A: Must be an administrator to view exclusions` - e o script so roda elevado; se um dia rodar sem admin, o efeito e degradar para a mensagem manual, nao para uma remocao errada.
- **Formatacao FAT32 de HD grande falhava com `Failed to write ... GetLastError()=5: Acesso negado` (`fat32Format.ts`, `fat32FormatGuard.test.cjs`) — v2.12.88**:
  - Print do usuario: um HD de 2 TB (`2199GB 4294965248 sectors`) chegando ate `Clearing out 1048416 sectors for Reserved sectors, fats and root cluster...` e morrendo na primeira escrita, cinco tentativas seguidas, terminando no conselho errado *"Feche qualquer programa ou janela do Explorer e tente novamente"*.
  - **A causa era nossa, e estava documentada como melhoria.** A 2.12.62 acrescentou `format fs=ntfs quick` antes do `assign letter` nos caminhos de `diskpart` **para suprimir o modal "Formate o disco na unidade X:"**. O efeito colateral: acima de 32 GB o `fat32format` recebe uma particao **com NTFS montado**, e ele escreve setores direto em `\\.\X:`. O Windows so libera escrita bruta num volume sem sistema de arquivos montado; com NTFS ali, a gravacao volta `ERROR_ACCESS_DENIED`. Uma correcao cosmetica quebrou a funcional.
  - Confirmado no binario, nao por suposicao: o `fat32format.exe` empacotado e a **versao 1.07**, e a ordem real do codigo (deduzida das referencias em `.text`) e `CreateFile` → `FSCTL_ALLOW_EXTENDED_DASD_IO` → `FSCTL_LOCK_VOLUME` → `zero_sectors`. O log do usuario tem o `FSCTL_ALLOW_EXTENDED_DASD_IO OK` e **nao** tem `Failed to lock device`, entao abrir, elevar e travar o volume funcionaram — o que sobrou foi a permissao de escrita no volume montado.
  - **Correcao:** acima de 32 GB a particao entregue ao `fat32format` e criada **sem sistema de arquivos** (`create partition primary` + `assign letter`, sem `format fs=ntfs`). O `convert mbr` continua, pelo motivo de sempre. Abaixo de 32 GB nada muda: ali quem formata e `Format-Volume`/`format.com`, que sao API de sistema de arquivos e nao sofrem o bloqueio.
  - **Ressalva assumida:** o modal *"Formate o disco na unidade X:"* volta a ser possivel por alguns segundos entre o `assign letter` e o fim do `fat32format`. E ruido; formatar e o que o usuario pediu, e ficar sem formatar nao e.
  - **A falha nao deixa mais o HD em RAW:** quando fomos nos que removemos o sistema de arquivos e a formatacao FAT32 ainda assim falha, a particao volta para NTFS antes do erro subir, em vez de devolver um HD de 2 TB que o Windows pede para formatar.
  - **Acesso negado parou de ser repetido cinco vezes.** Repetir so fazia sentido para `GetLastError()=32` (handle preso, que pode ser liberado); com o `=5` o volume ja tinha sido aberto e travado, entao a repeticao identica gastava ~10 s para reproduzir o mesmo erro. Agora a primeira ocorrencia interrompe o laco e anexa ao log o estado real do disco (`IsReadOnly`, `IsOffline`, `BusType`, modelo) e se a politica **"Discos removiveis: negar acesso de escrita"** esta ativa — as tres causas que sobram quando o bloqueio nao e handle preso.
  - **A mensagem parou de mentir.** O texto de acesso negado nao fala mais em fechar o Explorer: aponta chave fisica de protecao no gabinete, politica de bloqueio de escrita e antivirus. E a saida nativa passa a ser convertida com `ForEach-Object { [string]$_ }` antes do `Out-String` — sem isso o `2>&1` entrega `ErrorRecord` e o log despeja `NativeCommandError`/`CategoryInfo`/`FullyQualifiedErrorId` por cima da mensagem, que foi exatamente o paredao ilegivel do print.
  - **Verificacao do remount deixou de ser uma espera unica de 500 ms.** O `fat32format` desmonta o volume no fim e quem remonta e o Windows; com a particao vindo de RAW, meio segundo era pouco e a checagem final acusava *“sistema de arquivos resultante foi indeterminado”* num disco que tinha formatado certo. Agora insiste ate 5 s pelo FAT32 antes de desistir.
  - **Limite do que foi verificado:** o diagnostico e do log e do binario, nao de reproducao — nao ha HD de 2 TB com o bloqueio aqui. A pre-formatacao NTFS e a unica coisa **sob nosso controle** que contraria a receita conhecida do `fat32format`, e sai; se na maquina do usuario a causa for protecao de escrita ou politica, o log agora diz qual das duas, em vez de mandar fechar o Explorer.
...
### Fixed
- **Jogo de dois discos continuava baixando so um disco: a tela de navegacao tinha o proprio agrupador, e ele estava atrasado em relacao ao backend (`BrowsePage.tsx`, `browseHandlers.ts`, `preload.ts`, `handlers.go`, `router.go`, `compat.go`)**:
  - Relato: *"muitos jogos de 2 discos estao baixando separados quando deveriam baixar juntos, esta sendo possivel baixar somente um disco ou pior jogos que so tem o disc 2, sendo impossivel jogar"*. As 2.12.85 e 2.12.86 mexeram no backend; o defeito principal estava do outro lado.
  - **Havia dois analisadores de disco independentes.** `models.ExtractDiscInfo` (Go) e `parseDiscGame` (renderer) decidiam separadamente quais linhas do catalogo sao discos do mesmo lancamento, e quem manda no botao *"Baixar todos os N discos"* e o do renderer. Medido sobre os catalogos empacotados (`hf_xbox360` + `xbox360` + `minerva_xbox360`, 6219 nomes deduplicados, 488 linhas com tag de disco): o backend deixava **91** lancamentos com um disco so, o renderer deixava **172** — quase o dobro. Duas causas, as duas so no renderer: `DISC_SUBTITLE_REGEX` era lista fechada e exigia a palavra literal `Disc` **dentro** do grupo, entao `(Install)` e `(Play)` nao casavam; e `DISC_TAG_REGEX` so aceitava parenteses, nunca colchetes, entao `[DVD2]`/`[Disc 2]` — o formato do `games.json` — nao eram vistos como multidisco.
  - **Correcao: o renderer parou de analisar nome de disco.** Novo `GET /browse/releases?platform=<p>` devolve o agrupamento pronto, calculado por `models.FindCompanionDiscs` — exatamente a funcao que o `/trigger` usa para enfileirar companheiros, entao a tela nao consegue mais listar um disco que a fila nao buscaria, nem separar dois que ela juntaria. `parseDiscGame` e seus quatro regexes foram removidos; `getBaseTitle`/`getComparisonKey`, que agrupam **versoes** do mesmo jogo para exibicao, continuam no renderer porque nao tem nada a ver com disco. Nao ha caminho offline que justifique um segundo analisador: o catalogo na tela veio desse mesmo servidor.
  - **`(Play)` e `(Game)` sozinhos passaram a contar como papel de disco (`compat.go`).** O GTA V no Redump e `(Disc 1) (Install)` + `(Disc 2) (Play)`: o ramo de palavra-chave tirava `(Install)` e deixava `(Play)`, entao as duas metades produziam titulos diferentes e so a clicada baixava — o jogo que motivou a 2.12.85 continuava chegando pela metade por outro motivo. O padrao ganhou um segundo ramo que exige a palavra como **grupo inteiro**. Conferido sobre os 49419 nomes de todos os catalogos empacotados: so `(Play)` (3 linhas, todas GTA V) e `(Game)` (3 linhas, Resident Evil 6 e uma demo) casam. Os nomes que apenas **contem** as palavras — `(Sega Game Toshokan)`, `(Game no Kanzume Vol. 1)`, `(Xbox 360 Game Bundle)`, `(Required for Play)` — continuam intactos, e ha teste dos dois lados; funde-los enfileiraria dois produtos diferentes como discos um do outro. Lancamentos com um disco so no catalogo xbox360: **91 -> 85**.
  - **O veredicto de completude parou de depender de qual disco foi clicado.** `MissingDiscNumbers` (extraida de `recordReleaseCompleteness`) le o `of N` de **qualquer** linha do lancamento, via `DeclaredDiscCount`, e nao so da linha primaria: o Redump declara `(Disc 1 of 3)` numa linha e `(Disc 2)` na outra, entao o mesmo lancamento era completo ou incompleto conforme a metade clicada. A tela de navegacao precisa do veredicto **antes** de o usuario escolher um disco, o que tornou a divergencia visivel.
  - **Aviso antes do download, nao depois.** Restam 73 linhas cujo disco companheiro nao existe em catalogo compativel — 39 so em outra regiao (casar instalaria o disco errado) e 34 em lugar nenhum. A 2.12.86 avisava na entrega, ou seja, depois de baixar varios gigabytes. Agora `/browse/releases` devolve `missing_discs` e o texto do aviso, o dialogo de download mostra o bloco vermelho com o disco que falta e **o botao fica desabilitado ate a confirmacao explicita** (`handleQueue` recusa tambem, para o caso de o botao ser contornado). O seletor de versoes marca a linha incompleta, que costuma estar ao lado de uma completa. O aviso vem de `incompleteReleaseWarning`, o mesmo que o `LogStatus` anexa na entrega, com teste comparando os dois textos — quem confirma antes nao pode ser informado de outra coisa horas depois.
  - **O que continua sem cobertura, de proposito:** discos que so diferem pela tag de idioma (7 linhas: Call of Duty Ghosts, Dragon's Dogma, Dishonored, Metal Gear Solid HD Collection, Dead Space 2). Ignorar o idioma no casamento faria o Dishonored (Europe) achar **dois** candidatos a Disc 2 em idiomas diferentes e baixar os dois — trocar meia release por um download redundante de varios gigabytes nao e melhoria. Esses casos caem no aviso. `(Volume 1)`/`(Volume 2)` (Kinect Nat Geo) e `(Rev 1)` (Oblivion GOTY USA) tambem ficam de fora: sao marcadores que **distinguem** lancamentos em geral, e remove-los quebraria o outro lado.
  - **Achados da revisao, corrigidos antes do commit:** (1) o handler IPC captura o proprio erro e responde `{ ok: false }` em vez de lancar, entao o `catch` do renderer nunca rodava e uma falha de `/browse/releases` virava "todo jogo tem um disco so" — sem aviso, com o botao liberado; agora ha estado `loading`/`ready`/`failed`, o botao so libera em `ready` e a falha aparece com "Tentar novamente". (2) `companionCatalog` chama `ScanTransferFolder`, que no Windows faz `os.Stat` de D: a Z:, e o renderer esperava por isso antes de mostrar a lista — um HD externo dormindo ou unidade de rede morta segurava o catalogo inteiro; a lista nao espera mais, so o botao. (3) O endpoint filtrava por `IsMultiDisc` antes de agrupar, mas o `FindCompanionDiscs` do `/trigger` casa **so pelo titulo do lancamento**: linha sem `(Disc N)` sob o mesmo titulo — `Call of Duty - Advanced Warfare (USA, Europe) (Game Disc)` + `(Install Disc)`, e as duas linhas soltas do Dragon's Dogma, 10 lancamentos nos catalogos empacotados — era enfileirada pela fila e escondida da tela; o filtro saiu e o teste passou a comparar contra o `FindCompanionDiscs` sobre o catalogo **inteiro**, com mutacao provando que ele pega o defeito. (4) `discEntry` nao levava o `Subtitle` que o `models.DiscInfo` ja calcula, entao o painel de discos imprimia o mesmo nome truncado duas vezes em vez de "Install"/"Play". (5) `ExtractDiscInfo` nao checava faixa do `of N`: `byte(300)` virava 44 e `of 255` fazia o laco `1..N` de `MissingDiscNumbers` estourar o `byte` e nunca terminar — agora vale 1..20, como o numero do disco.
  - **Encontrado e nao corrigido, por precisar de decisao:** com a tela espelhando a fila, dois excessos **pre-existentes** do `/trigger` deixaram de ser invisiveis. (a) `fukikaeban`/`jimakuban` (dublado/legendado) entraram na lista de papeis de disco de `discSubtitlePattern` na 2.12.86, entao `(Fukikaeban)` e `(Jimakuban)` colapsam no mesmo titulo e a fila **ja** baixava os dois — sao versoes de audio, nao discos; sao **5** lancamentos, todos Call of Duty japoneses. (b) Quando dois catalogos listam o mesmo disco com nomes diferentes (`(Disc 1) (Game Disc)` do Redump e `(Game Disc)` do outro provedor), os dois entram no mesmo lancamento e a fila baixa o disco duas vezes: **4** lancamentos em 275 (Dragon's Dogma, Star Ocean The Last Hope, dois Call of Duty). Nos dois casos a fila se comporta hoje como sempre se comportou; o que mudou e que a tela passou a dizer quantos discos ela vai buscar, em vez de mostrar um numero menor. Corrigir qualquer um dos dois altera o **enfileiramento**, com risco de regressao no sentido oposto (deixar de baixar um disco de verdade), e ficou fora deste commit de proposito.
  - Testes: `compat_test.go` ganhou os pares GTA V/Resident Evil 6 no teste de agrupamento, tres pares que **nao** podem fundir no teste inverso, e dois testes novos para `MissingDiscNumbers`/`DeclaredDiscCount` (invariancia ao disco primario, e os extremos completo/disco solto). `release_completeness_test.go` ganhou a cobertura do endpoint: agrupamento igual ao da fila, jogo de um disco ausente da resposta, `missing_discs` do disco solto, igualdade entre o aviso do browse e o da entrega, e 400 sem `platform`. Suites: Go build/vet/test sem falhas, `tsc` limpo, Electron 168 testes, 0 falhas.


## [2.12.86] - 2026-09-09

### Fixed
- **Auditoria de completude: multidisco tinha mais tres formas de chegar pela metade (`compat.go`, `handlers.go`, `app.go`, `huggingface.go`, `pipeline.go`, `minerva.go`, `digital.go`, `stage_checkpoint.go`)**:
  - A 2.12.85 consertou o caminho XEX. Varrendo os cinco pipelines e os catalogos empacotados atras da promessa *"jogo de dois ou mais discos tem de vir sempre completo"*, apareceram tres buracos independentes.
  - **1. O disco de instalacao so era recuperado no ramo XEX.** `installCompanionDiscContent` foi acrescentado aos ramos **GOD**, **ISO** e **content package** dos quatro pipelines de arquivo — o mesmo defeito da 2.12.85 morava neles. `extractISOResilient` passou a devolver tambem o diretorio da extracao, que os chamadores nao tinham: sem ele nao havia como enxergar o que veio ao lado da ISO. No ramo GOD a chamada vem **depois** de `FindGODPackage`, que move (`os.Rename`) o pacote para `_god_prepared/`; antes dela o proprio jogo seria instalado duas vezes, uma como jogo e outra como conteudo.
  - **2. Dois discos do mesmo lancamento nao se reconheciam.** `FindCompanionDiscs` agrupa por titulo da release, e `discSubtitlePattern` era uma lista fechada de frases exatas. Os catalogos reais trazem texto livre: `(Single-Player Campaign)`, `(Install-Multiplayer)`, `(Single-Play)`, `(Multiplay-COOP)`, `(Campaign Part 1 & Multiplayer)`, `(Additional Content Packs Install Disc)`, `[DLC]`. Nenhuma casava, entao o Disc 1 e o Disc 2 produziam titulos diferentes e so o disco clicado era baixado — Battlefield 3, Battlefield Hardline, Borderlands GOTY e Borderlands 2 GOTY entre eles. O padrao passou a casar **palavra-chave de papel em qualquer posicao** dentro do grupo. Medido com o proprio codigo Go sobre os catalogos empacotados: lancamentos multidisco que nao agrupavam cairam de **25→19** (`games.json`), **83→63** (`xbox360.json`) e **113→87** (`minerva_xbox360.json`) — 52 lancamentos recuperados. Os grupos que **distinguem** releases (`(USA, Europe)`, `(En,Fr,De)`, `(Triple Pack)`, `(Volume 1)`) nao contem nenhuma dessas palavras e continuam no titulo; ha teste para os dois lados.
  - `ExtractDiscInfo` passou a ler o subtitulo do nome **sem** a tag de numero: `(Disc 1)` tambem e um grupo que nomeia papel de disco, entao com o padrao novo o numero do disco viraria o papel.
  - **3. O que sobra nao tem conserto por codigo — e passou a ser dito.** Mesmo depois do agrupamento, 169 lancamentos continuam com um disco so: o companheiro **nao existe** em catalogo nenhum sob titulo compativel. `Alien - Isolation (USA) (Disc 2)` so tem Disc 1 como `(USA, Europe)`; `Call of Duty - Ghosts (France) (Disc 1)` so tem Disc 2 como `(Japan)`. Casar entre regioes instalaria o disco errado. Novo `recordReleaseCompleteness` confere, no enfileiramento, se os numeros de disco formam 1..N; quando nao formam, grava o aviso em `App.IncompleteRelease` para **todos** os discos daquele lancamento, e `LogStatus` o anexa a mensagem de entrega. Como `LogStatus` e o funil unico de todos os pipelines, nenhum caminho de entrega — GOD, XEX, content, FTP ou local — consegue reportar "pronto" sobre meia release em silencio. Mensagens de progresso nao sao alteradas.
  - Um `Disc 1` sozinho conta como incompleto igual a um `Disc 2` sozinho: linha de catalogo rotulada "Disc 1" nunca e lancamento de um disco. O aviso e removido quando o job sai da fila.
  - **O que continua sem cobertura, de proposito:** arquivo unico com **duas ISOs** dentro — `extractISOResilient` usa a primeira e as outras seguem ignoradas; converter cada uma exigiria nome e pasta proprios por disco, que e a decisao que o catalogo normalmente ja toma em linhas separadas. E busca de disco companheiro **entre catalogos** por titulo normalizado, que resolveria boa parte dos 169 (o `Alien Isolation [RF][DVD2]` de `games.json` supre o `(USA)` que falta no Redump), mas so com regra de equivalencia de release que ainda nao existe. Os dois casos hoje caem no aviso em vez de passarem em silencio.
  - Testes: tres em `compat_test.go` travam o agrupamento (seis pares reais que nao agrupavam, tres pares que **nao** podem fundir, e a ordem de leitura do subtitulo) e sete em `release_completeness_test.go` cobrem a deteccao e o anexo no `LogStatus`. O anexo foi conferido por mutacao: removido o bloco do `LogStatus`, falha ele e so ele. Suites: Go sem falhas, Electron 168 testes, 0 falhas.

## [2.12.85] - 2026-09-09

### Fixed
- **Jogo de dois discos chegava ao console pela metade e com o nome do disco (`multi_disc.go`, `companion_content.go`, `huggingface.go`, `minerva.go`, `pipeline.go`, `digital.go`, `handlers.go`, `localGameScannerService.ts`)**:
  - Relato: em **Jogos instalados**, o GTA V aparecia so com o Disc 2 e chamado "Disc 2". O pedido em aberto e o oposto — jogo de dois ou mais discos tem de chegar **completo e pronto para jogar**.
  - **O disco de instalacao era jogado fora.** O rip do GTA V no catalogo HuggingFace e um unico arquivo de 15,4 GB (`Grand.Theft.Auto.5.EUR.X360-ZTM.rar`) que traz a release inteira: o disco jogavel **e** o disco de instalacao, que e o que o GTA V precisa para dar boot. `helpers.FindXEXFolder` devolve a **primeira** pasta com `default.xex` e para ali (`io.EOF`); os cinco caminhos de instalacao XEX gravavam so essa pasta e o resto do arquivo extraido ia junto com a limpeza do scratch. Vale para toda release do genero — Metal Gear Solid V, Battlefield 4, Alien: Isolation, Forza Motorsport 3 e 4.
  - **Correcao:** `FindCompanionContentPayloads` varre o extraido atras de arvores `Content/0000000000000000/<TitleID>/<tipo>` **fora** da pasta jogavel, e `installCompanionDiscContent` grava cada uma no destino antes do jogo — local (`InstallContentLocal`) ou FTP (`TransferContent`) — as duas passaram a receber o tipo de conteudo em vez de assumir `00000002`. O TitleID sai do **cabecalho do pacote**, nao do nome da pasta: instalador de fabrica endereca a propria arvore como `FFED2000` e o console procura pelo ID real do jogo. Pacotes assinados sob o titulo de sistema (`FFFE07DF` — Kinect, voz, dashboard) sao descartados, senao dado de painel iria parar onde se espera um jogo.
  - **A gravacao do conteudo vem antes da do jogo**, de proposito: assim uma falha para o job enquanto o fallback ainda pode procurar uma release que caiba no destino, em vez de deixar meio jogo gravado e reportado como pronto. Num pendrive FAT32 cujo pacote de instalacao passe de 4 GB, `copyTreeLocal` recusa antes de escrever qualquer byte e a cadeia troca de provedor atras de uma versao GOD.
  - **O nome vinha da pasta de dentro do arquivo.** Rip de multidisco chama a pasta raiz de `Disc2`, entao o destino virava `Disc2 - 545408A7` e a biblioteca listava "Disc2". `xexDestinationName` troca esse marcador pelo titulo da release (`Grand Theft Auto 5 - 545408A7`) e deixa intacto o nome de rip que ja identifica o jogo (`Gears.of.War.1.USA.X360-ZTM`), que a lista de instalados ja sabe limpar.
  - **Pastas ja gravadas nao sao migradas** — mesma ressalva da 2.12.81. Para elas, `localGameScannerService` passou a tratar `Disc2`/`DVD 1`/`Game Disc` como ausencia de nome e resolver o titulo pelo TitleID, entao a instalacao que o usuario ja tem passa a aparecer como **GTA V** sem reinstalar. O jogo continua incompleto: so uma nova instalacao traz o disco que faltou.
  - **Disco companheiro nao era enfileirado no caminho padrao (`handlers.go`)**: `/trigger` so chamava `enqueueCompanions` nos ramos `source=huggingface` e Internet Archive. O ramo **unificado** — o default do aplicativo e do Aurora, que e por onde quase todo download passa — retornava sem enfileirar nada, assim como o Minerva explicito e a ISO local da pasta Transfer. Quando os discos sao entradas separadas do catalogo, so vinha o disco clicado. A promessa registrada na 2.12.66 (*"o endpoint /trigger enfileira automaticamente os discos companheiros"*) valia para dois dos cinco ramos; agora vale para os cinco. Duplicata nao ocorre: `RegisterGameJob` guarda um token por nome e o lancamento mais novo invalida o anterior.
  - **Fora do escopo:** sem destino registrado (`/trigger` sem `/register`) o job so empacota a pasta jogavel para instalacao manual — o `godsend.ini` tem uma secao e um tipo so, e um manifesto `xex` nao tem onde nomear destino de conteudo. Fica registrado no log do job.
  - Testes: oito casos em `multi_disc_test.go` (disco de instalacao ao lado da pasta jogavel, placeholder `FFED2000` resolvido pelo cabecalho, pasta do jogo ignorada, pacote de sistema descartado, pasta de tipo ausente, arquivo solto nao enderecado, pacote de sistema que vem antes do real na ordem do `ReadDir`, e o marcador de disco separado de titulos como `Discworld Noir`), tres em `companion_content_test.go` para o nome do destino e dois em `localGameScannerService.test.cjs`. Dois foram conferidos por mutacao e falham eles e so eles: o de `Disc2` no scanner (revertendo a condicao no `.js` compilado) e o da ordem do `ReadDir` (voltando a aceitar o primeiro pacote lido). Suites: Go sem falhas, Electron 168 testes, 0 falhas.
...
### Fixed
- **Dois jogos de discos diferentes gravados na mesma pasta, um sobrescrevendo o outro (`helpers.go`, `pipeline.go`, `minerva.go`, `digital.go`, `huggingface.go`)**:
  - Relato: seis tarefas na fila marcadas **"Pronto — Gravado no dispositivo!"**, mas a tela **Jogos Instalados** mostrava só parte delas. Os downloads estavam certos; o destino da gravação é que era ambíguo.
  - `folderName` para instalação XEX saía de `filepath.Base(xexFolder)`, ou seja, **o nome da pasta que veio dentro do arquivo baixado**. Rips de jogos de dois discos trazem a pasta raiz chamada `Disc2` — e tanto Forza Motorsport 3 quanto GTA 5 são jogos de dois discos. Como `InstallXEXLocal` montava o destino como `<root>/<xexSub>/<folderName>` **sem TitleID** (diferente de `InstallGameLocal`, que sempre usa `"%s - %s"` com o TitleID), os dois títulos resolviam para `E:\games\Disc2`.
  - `copyTreeLocal` mescla no destino existente e trata arquivo de mesmo tamanho como já gravado — semântica de retomada, correta para reinstalar o mesmo jogo, que aqui tornava a fusão **silenciosa**. No pendrive do relato, GTA 5 gravou 7,69 GiB às 22:11–22:17 e Forza Motorsport 3 gravou 1,88 GiB na mesma pasta às 23:03–23:04, sobrescrevendo `default.xex` e `nxeart` às 23:04:50. Os dois jogos ficaram inutilizáveis, e as duas tarefas relataram sucesso.
  - **Correção:** novo `helpers.XEXFolderName(xexFolder, folderName)` acrescenta o TitleID ao nome vindo do arquivo, replicando o layout `"<nome> - <TitleID>"` que o caminho GOD já produzia. O TitleID vem de `FindTitleIDInDir`, que a mesma função já chamava — **depois** da gravação, em `LogLocalComplete` — para reportar a conclusão: o dado necessário já existia, era usado tarde demais.
  - Aplicado nos **seis** pontos de derivação de `folderName` (`pipeline.go` ×2, `minerva.go` ×2, `digital.go`, `huggingface.go` ×2), e não em cada função de gravação, porque corrigir na origem cobre também o **FTP** — `ftp/client.go` monta `/<drive>/<xexSub>/<folderName>` com a mesma ambiguidade, e a mesma colisão aconteceria no HD do console.
  - Nome que já termina em TitleID não é duplicado; TitleID indeterminável mantém o nome anterior, sem regressão.
  - **Ressalvas:** a correção impede colisões **novas** — pastas já gravadas mantêm o nome antigo, e não há migração. Uma pasta já fundida (`Disc2` com dois jogos) não é recuperável separando arquivos, porque o `default.xex` do primeiro jogo foi sobrescrito; o caminho é apagar e regravar. Instalar de novo um jogo XEX que já estava no dispositivo cria a pasta com o novo nome ao lado da antiga.

- **Jogo íntegro sumindo de "Jogos Instalados" por causa dos pacotes Kinect que ele carrega (`localGameScannerService.ts`)**:
  - `probeStfsTitleId` varre a pasta do jogo e adota o TitleID do **primeiro** container STFS que encontra, sem verificar se ele pertence ao jogo. Vários jogos embarcam pacotes de fala e Kinect (`Database.xmplr`, `NuiIdentity.bin.be`, `NuiHeadOrientation.bin.be`, `nuisp*`) que a Microsoft assina sob o título de **sistema** `FFFE07DF` — no Forza Horizon 2 Presents Fast & Furious são 19 deles, na raiz da pasta.
  - A pasta herdava `FFFE07DF`, e aí o guarda "pular dados de system update" descartava o **jogo inteiro** — mesmo com `default.xex` na raiz, que prova ser jogo e não dado de sistema. Confirmado executando o scanner real contra o pendrive: `titleId = FFFE07DF` → `REJEITADO`.
  - **Correção:** `FFFE07DF` entra na mesma lista de IDs que o probe já ignorava (`00000000`, `FFFFFFFF`) — nunca é o TitleID de um jogo. O guarda continua valendo para pastas que **declaram** ser de sistema (nome, subpasta hex ou `godsend.ini`), que é o caso legítimo que ele existe para cobrir; `E:\Content\0000000000000000\FFFE07DF` segue ignorado.
  - Efeito colateral favorável: sem parar no primeiro pacote de sistema, o probe continua a varredura e encontra o container real do jogo. O Forza Horizon 2 F&F passou a ser listado **com o TitleID correto (`4D530AA4`)**, não apenas listado.

- **Tamanho de jogo subnotificado em ordens de grandeza (`localGameScannerService.ts`)**:
  - `getDirectorySizeBytes` parava em `depth > 3` e devolvia `0` para o que estivesse abaixo. Árvores de jogo Xbox são mais fundas que isso (`media/tracks/<pista>/<asset>`, `Content/0000000000000000/<TitleID>/<tipo>/`), então o corte não aproximava o tamanho — descartava a maior parte dele.
  - Medido no pendrive do relato: pasta de **9,65 GiB** exibida como **71,5 MB**. Limite elevado para `MAX_SIZE_SCAN_DEPTH = 16`; a mesma pasta passou a reportar 9,65 GiB.
  - Consequência que motivou a investigação: **"Ocupado"** na `UsbGamesPage` é a soma dos jogos escaneados, enquanto **"Livre"** vem do sistema operacional. Todo jogo descartado ou subnotificado saía também do total, e foi a diferença entre os dois números (33,88 GiB realmente em uso contra 21,51 GiB somados) que denunciou os três defeitos.

- **Simetria entre os dois lados da mesma regra, apontada em revisão**: a exclusão do título de sistema existia só no scanner (TypeScript). No Go, `FindTitleIDInDir` não tinha guarda nenhuma e varre em ordem lexical — `Database.xmplr` vem antes de `default.xex` —, então um `default.xex` que a extração de TitleID não conseguisse ler faria a instalação virar `Games\<nome> - FFFE07DF`, uma pasta que o próprio scan recém-corrigido descarta como dado de sistema. `XEXFolderName` passou a ignorar `SystemTitleID` e manter o nome do arquivo, aceitando o risco (bem mais raro) de colisão em vez do jogo invisível garantido.
  - Ainda em revisão: excluir o título de sistema dentro de `probeStfsTitleId` tornava **inalcançável** o guarda que descarta dados de dashboard. O probe agora prefere o container do próprio jogo e só devolve o de sistema quando é o único que existe; e o guarda deixou de ser incondicional — dado de sistema não tem executável próprio, então uma pasta com `default.xex` ou `godsend.ini` é jogo que apenas **embute** pacotes Kinect, e o que se descarta é o TitleID enganoso, não o jogo. Sem isso, um jogo cujos únicos containers fossem pacotes Kinect voltaria a sumir da lista.
  - **Limite de nome FATX**: o sufixo acrescenta 11 caracteres, e o sistema de arquivos do HD interno do console corta em 42. Como `TransferXEX` descarta o resultado de `MkdirAll`, um nome recusado viraria um job de FTP falhando igual em toda retentativa, sem dizer a causa. `XEXFolderName` corta a base preservando o TitleID — que é justamente o que garante a unicidade — e apara espaço ou ponto finais, que o Windows descartaria do nome da pasta.

- **Ressalva conhecida (não corrigida)**: `getDirectorySizeBytes` roda síncrono no processo principal do Electron, e subir o corte de 3 para 16 aumenta o trabalho numa árvore grande. Não foi imposto teto de arquivos ou de tempo **de propósito**: qualquer um dos dois devolve tamanho errado em silêncio, que é exatamente a classe de defeito corrigida aqui. A solução correta — tirar a varredura do processo principal — é refatoração, não conserto de bug. O risco fica limitado pelos predicados de entrada de `scanDriveRoot` e pelo cache de 15 s do scan.

- **Verificação**: `XEXFolderName` coberto por seis testes em `xex_folder_name_test.go` — incluindo dois rips `Disc2` de títulos diferentes resolvendo para destinos distintos, o título de sistema ignorado e o corte FATX preservando o TitleID. Cinco testes novos em `localGameScannerService.test.cjs`, com containers STFS sintéticos, fixam os defeitos do scanner nos dois sentidos: jogo com pacotes de sistema é listado, dados de dashboard continuam descartados. Os três primeiros foram conferidos por mutação (revertendo as duas linhas no `.js` compilado, falham eles e só eles). Suítes: Go sem falhas, Electron 163 testes, 0 falhas. Os três defeitos foram confirmados e as correções revalidadas executando o scanner real contra o pendrive do relato.

## [2.12.80] - 2026-09-08

### Added
- **Aviso na seleção quando o disco passa de 2 TB (`BadAvatarUsbPage.tsx`)**: fecha a lacuna registrada na 2.12.79.
  - O cliente que abriu o chamado formatou um HD de 4 TB, ficou com 2 TB e perguntou se o programa tinha estragado o disco. Nada na tela dizia que aquilo ia acontecer: `assessDeviceSafety` tem piso (`INVALID_CAPACITY`) e nenhum teto, então um disco acima de 2 TiB era aceito em silêncio e a perda só aparecia depois da formatação — tarde demais para devolver o HD à loja.
  - Selecionado um disco com `sizeBytes` acima de 2 TiB, o card **Dispositivo conectado** passa a mostrar uma faixa âmbar com os números reais daquele disco: quanto ele tem, quanto sobra inacessível e por quê. O texto diz explicitamente que **não é defeito e que uma segunda partição não recupera o espaço** — as duas conclusões erradas que o cliente tirou sozinho — e que a capacidade volta inteira se o disco for reformatado para uso no computador.
  - **É aviso, não bloqueio.** O disco continua selecionável e preparável: 2 TB funcionam normalmente e o usuário pode ter feito essa escolha de propósito. Por isso o teto ficou fora de `assessDeviceSafety`, cujos códigos são todos impeditivos (`allowed: false`) — colocá-lo lá recusaria um disco que funciona.
  - A checagem usa `sizeBytes`, que no caminho de enumeração física é o tamanho do **disco** (`Get-Disk .Size`), não o da partição. É o que permite avisar mesmo quando o HD de 4 TB já chega particionado em 2 TiB e o volume, sozinho, não denunciaria nada.
  - `formatBytes()` ganhou a faixa de TB. Antes o teto era GB, então um HD de 4 TB aparecia na lista de unidades como **"3725.3 GB"** — ilegível justamente na classe de dispositivo que o aviso trata.
  - Limiar conferido nos tamanhos comerciais: HD de "2 TB" tem 1,82 TiB e **não** dispara o aviso (cabe inteiro); 4 TB mostra "3.6 TB" com 1.6 TB inacessíveis; 8 TB, 5.3 TB. Sem falso positivo no tamanho que a documentação recomenda comprar.
  - A tela conta em potências de 2 e rotula "TB", como o Windows; o anúncio da loja conta em potências de 10. O mesmo HD é "4 TB" na caixa e "3.6 TB" na tela, e a perda é "1,8 TB" ou "1,6 TB" conforme a unidade. `docs/CAPACIDADE-E-ESPACO.md` ganhou a tabela de conversão para que atendimento e tela não citem números diferentes do mesmo disco — que era exatamente o risco no aviso criado para encerrar esse chamado.
  - **Ressalva:** o aviso está só na tela de preparação (`BadAvatarUsbPage`), que é onde a formatação é escolhida. `UsbGamesPage` mostra a mesma classe de dispositivo e não recebeu a faixa. O `renderer/` está fora do `tsconfig` e não tem suíte de testes neste repositório, então a verificação foi build do Vite mais conferência do limiar e da aritmética fora da tela.

## [2.12.79] - 2026-09-08

### Fixed
- **"O Windows demorou demais para listar os dispositivos USB" e "O dispositivo mudou desde a seleção" em rajada, no mesmo cliente (`windowsUsbDeviceService.ts`)**:
  - Duas mensagens, um mecanismo só. O relato ("toda hora dá essa mensagem, depois deu essa outra") descreve o **vaivém entre os dois caminhos de enumeração**, não duas falhas independentes.
  - `ENUMERATE_REMOVABLE_SCRIPT` existe, segundo o próprio comentário acima dele, porque *"Storage Management pode bloquear indefinidamente depois de uma desconexão USB instável"* — e então **chamava `Get-Volume`**, que é Storage Management, uma vez por unidade listada, dentro do laço. O fallback dependia exatamente do serviço do qual ele era o fallback. Medido nesta máquina, saudável e com cache quente: `Get-Volume` na primeira chamada = **1248 ms** de um orçamento de 5000 ms (`DriveInfo` = 2 ms, `mountvol` = 44 ms, `Add-Type` = 187 ms). Na máquina do cliente, com o serviço travado, ele consome o orçamento inteiro → *timeout* → cai para `ENUMERATE_USB_SCRIPT`, que é 100% Storage Management e estoura os 12 s → **primeira mensagem**.
  - Quando só um dos dois lados falha, o caminho de enumeração **troca entre a listagem e a revalidação**, e os dois caminhos descrevem o mesmo pendrive com identidades diferentes (o nativo não tem número de disco, serial nem modelo). O fingerprint de um nunca é reproduzido pelo outro → **segunda mensagem**, acusando o usuário de ter trocado um pendrive que nunca saiu da porta.
  - A tolerância que deveria cobrir isso (`createSyntheticRemovableFingerprint`) tenta `partitionSizeBytes` e `sizeBytes` do dispositivo **atual**, mas o fingerprint sintético carrega `DriveInfo.TotalSize`, que não é nenhum dos dois. Medido em volumes reais: `Get-Partition.Size` menos `DriveInfo.TotalSize` = 3072 e 4096 bytes (NTFS; em FAT32 a diferença é maior, porque `TotalSize` exclui FATs e setores reservados). Num SHA-256, 3 KB de diferença é o mesmo que 3 TB. **O teste que cobria essa transição passava porque usava o mesmo número dos dois lados** — combinação que não ocorre em disco real.
  - **Correção 1:** o script nativo não chama mais Storage Management. As dicas de integridade saíram dele para `annotateRemovableHealth()`, um processo próprio com teto de 3 s, `try/catch` e resultado descartável: serviço travado passa a degradar o aviso *"sistema de arquivos com inconsistências"* em vez de derrubar a lista de dispositivos. Health/repair não entram em `createDeviceFingerprint` nem em `assessDeviceSafety` — custam uma sugestão, não uma verificação de segurança. A sonda é **opt-in** (`enumerateSafeWindowsUsbDevices(true)`, só a partir de `listFat32UsbDrives`): a mesma função é o caminho de revalidação, que `createThrottledUsbTargetRevalidator` dispara **a cada 10 s durante toda a gravação**, e ligá-la ali recolocaria o `Get-Volume` no caminho quente — cerca de 120 sondas numa preparação de 20 minutos, exatamente o que a correção existe para evitar.
  - **Correção 2:** `requireSafeWindowsUsbTarget` já reexecutava o script físico quando a revalidação sintética falhava; agora existe também o sentido inverso, mas **deliberadamente assimétrico**, na nova política pura `planRevalidationRetry()`. Linha nativa → sempre reexecuta o físico, que só pode apertar o veredito (ele traz `isReadOnly`, `isOffline`, `isBoot`/`isSystem` e contagem de partições reais). Linha física → só reexecuta o nativo se o veredito físico foi `allowed` **e** a unidade é `Removable`. As duas condições têm motivos distintos: o script nativo fixa aquelas flags no valor permissivo e a contagem de partições em 1, então resgatar depois de um bloqueio devolveria como segura uma unidade recém-recusada (pendrive com trava de gravação, ou com uma segunda partição montada); e como o nativo pula tudo que não é `Removable`, para um HD USB (`Fixed`) a retentativa não produziria linha nenhuma — só um processo e um timeout antes do mesmo erro. Uma troca real de pendrive continua recusada nos dois sentidos: um fingerprint físico carrega serial e modelo reais e nunca é igual a um hash sintético.
  - Reproduzido em código antes da correção: listado nativo → revalidado físico e listado físico → revalidado nativo, ambos com os tamanhos reais de um pendrive de 64 GB, dão a mensagem do print. Em `deviceSafetyPolicy.test.cjs`, um teste novo fixa esse comportamento e explica por que o resgate por reexecução é necessário, e mais quatro cobrem `planRevalidationRetry()` — incluindo os dois casos em que a retentativa é recusada. Verificado por mutação: inverter a decisão de caminho derruba 4 testes. O teste antigo ganhou o aviso de que seus tamanhos iguais são artificiais. Suíte: 158 testes, 0 falhas.
  - **Ressalva:** sem pendrive removível conectado, o script físico continua sendo executado sempre — HDs USB aparecem como `Fixed`, não `Removable`, e só o caminho físico os encontra. Numa máquina com Storage Management travado *e* nenhum pendrive na porta, a primeira mensagem continua possível; ela deixa de aparecer com o dispositivo conectado, que é o cenário do relato. Detalhamento, com as medições, em [`docs/bugs/closed/enumeracao-usb-timeout-e-dispositivo-mudou-desde-a-selecao_2026-09-08T00-00.md`](docs/bugs/closed/enumeracao-usb-timeout-e-dispositivo-mudou-desde-a-selecao_2026-09-08T00-00.md).

### Added
- **Teto de 2 TiB por disco USB documentado (`docs/CAPACIDADE-E-ESPACO.md`, `README.md`, `docs/features.md`, `AGENTS.md`, as duas bases de suporte)**:
  - Relato de suporte: cliente formatou um HD de 4 TB e ficou com 2 TB, perguntando se o programa havia estragado o disco. Não havia **uma linha sequer** sobre isso em nenhum documento do projeto — nem no README, nem no `features.md`, nem nas bases de atendimento —, então cada ocorrência virava um chamado investigado do zero.
  - Não é defeito nem otimização pendente: **dois limites independentes caem no mesmo número**. A entrada da tabela MBR guarda a contagem de setores em 32 bits (2³² × 512 B = 2 TiB) e o campo `BPB_TotSec32` do FAT32 também é de 32 bits. Um HD de "4 TB" tem 3,64 TiB; sobram ~2,2 TB decimais, que o Windows rotula como 2,00 TB. Criar uma segunda partição com o resto **não funciona** — o MBR não consegue endereçar além de 2 TiB, o espaço não fica livre, fica fora do que o esquema descreve.
  - Os quatro caminhos de `diskpart` em `fat32Format.ts` usam `convert mbr` **de propósito**: o Xbox 360 lê FAT32 em MBR, e trocar por GPT devolveria a capacidade no Windows ao custo de tornar o disco ilegível no console. Registrado em `AGENTS.md` como invariante, para que a próxima leitura do código não trate o MBR como bug a corrigir.
  - Documentado também o caminho exato que produz o sintoma: acima de 32 GB o fluxo principal formata a **partição existente** e só cai no `diskpart` quando o `fat32format` falha — que é o que acontece com um HD de 4 TB chegando em GPT com partição de 3,64 TiB.
  - Nova pergunta 7 no MÓDULO 5 das duas bases de suporte, com resposta pronta em linguagem de cliente e um aviso ao atendente de que não há conserto. Recomendação de compra registrada: 2 TB é o teto útil por disco; acima disso, dois discos.
  - **Lacuna deixada em aberto, deliberadamente:** o app continua sem avisar nada. `assessDeviceSafety` só tem piso (`INVALID_CAPACITY`, abaixo de 1 GB) e nenhum teto, então um disco maior que 2 TiB é aceito em silêncio e o usuário só descobre a perda depois de formatar. Um aviso na seleção resolveria a categoria inteira de chamado; está registrado nos quatro documentos, mas não implementado nesta versão.

## [2.12.78] - 2026-09-06

### Changed
- **Ordem de resolução dos executáveis do sistema invertida: %PATH% primeiro, `System32` como fallback (`windowsSystemExecutables.ts`, `fat32Format.ts`)**:
  - A 2.12.76 tinha fixado o contrário (System32 primeiro). Invertido a pedido: o %PATH% volta a ser autoritativo, e quem apontar a máquina para outra instalação continua mandando; o caminho canônico só entra quando a busca não acha nada.
  - A busca é feita **sem gastar um `spawn` que falha**: `resolveOnPath()` varre as entradas do `%PATH%` com `existsSync` e devolve o caminho absoluto encontrado — mesmo resultado que o `spawn` teria, mas observável e registrável em log. É o padrão que o backend Go já usa para o `aria2c` (`torrent.go:142`). A varredura ignora o diretório atual, que o `CreateProcess` também consultaria; a diferença só aparece com um `powershell.exe` largado no cwd, e ignorá-lo é preferível.
  - Cadeia final: `%PATH%` → `%SystemRoot%\System32\…` → nome simples (que ainda cobre o caso do cwd e o não-Windows). A regra que originou a correção **não mudou**: nunca passar nome simples direto para `spawn`, porque aí o ENOENT é a única resposta possível quando o PATH está quebrado.
  - **Ressalva de segurança, registrada porque a inversão a reintroduz:** com o %PATH% na frente, um `powershell.exe` plantado numa pasta anterior da lista é executado no nosso lugar — inclusive em `runPs1Elevated`, que pede UAC. Um PATH de usuário é gravável sem elevação, então isso converte "escrever no PATH do usuário" em "obter elevação pelo nosso prompt". Fixar só o `Start-Process` interno **não** resolveria: o pai já seria o interpretador sequestrado. Se essa exposição pesar mais que a flexibilidade, a volta é uma linha em `system32Exe()`.
  - `POWERSHELL_EXE_PS_EXPRESSION` foi removido. No lugar, `buildElevationScript(ps1Path, psExe)` recebe o interpretador **já resolvido pelo processo pai** e o cita literal — o `Start-Process` aninhado não repete a busca por conta própria, então pai e filho elevado são sempre o mesmo PowerShell.
  - Esse script de elevação não tinha teste nenhum. Agora tem: caminho do interpretador citado literal, aspas simples escapadas (`It''s Here`) e sintaxe validada pelo `[Parser]::ParseInput` do próprio PowerShell, o mesmo validador que o `buildGuardedWindowsFat32Script` já usava.
  - Verificado nos três cenários: PATH normal → resolve pelo PATH; PATH com uma cópia própria antes do System32 → a cópia vence; PATH quebrado (`C:\NaoExiste`) → cai no System32 e os quatro call sites reais (PowerShell, `net session`, `cmd`+`chkdsk`, enumeração de USB) continuam funcionando. Suíte: 153 testes, 0 falhas.

## [2.12.77] - 2026-09-06

### Added
- **Falha ao listar dispositivos passa a reportar telemetria, com diagnóstico de PATH junto (`badAvatarHandlers.ts`, `windowsSystemExecutables.ts`)**:
  - O `spawn powershell.exe ENOENT` da 2.12.76 chegou até nós como **uma foto de um monitor**, e isso não foi acaso: a telemetria só dispara em `uncaughtException`/`unhandledRejection` (`bootstrap.ts:25,55`) e no `window.onerror` do renderer. A falha da enumeração é capturada pelo `try/catch` de `badAvatarHandlers.ts` e devolvida como `{ ok:false }` — comportamento correto, que por isso mesmo passava longe do reporte. Qualquer falha futura dessa tela seria igualmente invisível.
  - `tools:badavatar-list-drives` agora chama `reportError` no `catch`. Respeita o kill-switch `getConfiguredErrorReporting()`, é deduplicado por `component|message` (clicar **Atualizar** dez vezes gera um reporte, não dez) e `reportError` é documentadamente incapaz de lançar — o handler não fica mais frágil por causa disso.
  - Novo `describeWindowsExecutableEnvironment()` grava no log e vai junto do reporte a linha que **decide entre as hipóteses** do bug anterior: `powershellNoDisco` separa "arquivo removido" (Windows debloatado) de "arquivo presente, PATH sem a pasta"; `pathTemSystem32`/`pathTemWindowsPowerShell` confirmam o segundo caso; e `pathChars` perto de 1024 ou 2047 aponta truncamento (`setx`, caixa de diálogo antiga) em vez de edição manual. `arch` responde se é build `ia32` sobre Windows 64-bit, onde o `System32` é redirecionado para o `SysWOW64` — pergunta que ficou em aberto na 2.12.76.
  - **O PATH sai com a pasta do perfil mascarada** (`%USERPROFILE%`): o nome da conta do Windows costuma ser o nome real da pessoa, o log completo já é anexado a todo reporte (`telemetry.ts:31`) e isso vai para um serviço externo. `arch` e build do Windows não precisaram ser adicionados — o campo `platform` já os carrega.
  - **Ressalva:** o reporte cobre *todas* as falhas de enumeração, inclusive o timeout de USB, que é comum e recuperável pelo usuário. Foi deliberado — filtrar por palpite foi exatamente o que deixou o ENOENT invisível — mas se o painel encher de timeout, o filtro certo é excluir só essa mensagem.
  - Testes: `windowsSystemExecutables.test.cjs` cobre o PATH quebrado (`pathTemSystem32=false` com `powershellNoDisco=true`, que é a assinatura do print) e o mascaramento do perfil. Suíte completa: 152 testes, 0 falhas.

## [2.12.76] - 2026-09-06

### Fixed
- **`spawn powershell.exe ENOENT` derrubava a lista de dispositivos em "Prepare seu pendrive ou HD" (novo `windowsSystemExecutables.ts`, `windowsUsbDeviceService.ts`, `badAvatarUsbService.ts`, `fat32Format.ts`, `driveRepairService.ts`, `fakeDriveProbeService.ts`, `autoUpdateService.ts`)**:
  - Print do usuário: o card **Dispositivo conectado** exibindo `spawn powershell.exe ENOENT` no lugar da lista, com o seletor de unidade vazio e o "Formatar antes" desabilitado.
  - Todo `spawn`/`spawnSync` do app passava o **nome simples** `"powershell.exe"`, que o Windows resolve pelo `%PATH%`. Em máquinas cujo PATH perdeu `%SystemRoot%\System32\WindowsPowerShell\v1.0` — editado à mão, truncado no limite de 2047 caracteres ou removido por imagem "debloatada" do Windows, comum no público que instala RGH — a chamada falha antes de executar qualquer script. Reproduzido com `PATH=C:\NaoExiste`: nome simples → `spawnSync powershell.exe ENOENT`; caminho absoluto → `OK`.
  - `enumerateSafeWindowsUsbDevices` engole a falha da enumeração nativa (`try/catch` que só registra em log) e **propaga** a da enumeração física; `badAvatarHandlers.ts:41` devolve `err.message` cru e o `BadAvatarUsbPage` mostra esse texto como `loadError`. Daí a mensagem interna do Node ter chegado à tela sem tradução.
  - O novo `infrastructure/windowsSystemExecutables.ts` resolve o interpretador em `%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe` (e `cmd.exe`, `net.exe`, `chkdsk.exe` em `System32`), mantendo o nome simples como último recurso — fora do Windows, ou quando o arquivo realmente não existe, o comportamento é o de antes, e não uma falha num caminho inventado. Os scripts `.ps1` deste repositório já resolviam `diskpart.exe`, `mountvol.exe` e `format.com` por `$env:SystemRoot` pelo mesmo motivo; faltava o próprio interpretador.
  - O `Start-Process powershell -Verb RunAs` **dentro** do script de elevação do formatador tinha a mesma exposição ao PATH e agora usa `Join-Path $env:SystemRoot`.
  - Corrigido junto: `isRunningAsAdmin()` chamava `net session` pelo nome, então nas mesmas máquinas o `tools:badavatar-is-admin` rejeitava e a disponibilidade do formatador FAT32 ficava indefinida em silêncio (o `Promise.allSettled` do `refreshDrives` absorve a rejeição).
  - Quando o PowerShell realmente não existe, a listagem passa a dizer o que falta e onde foi procurado, em vez de repetir `spawn powershell.exe ENOENT`.
  - No `repairDrive`, o caminho do `chkdsk` entra **sem aspas** na linha do `cmd /c`: o Node escapa um `"` interno como `\"`, que o `cmd` recusa com *"não é reconhecido como um comando interno ou externo"*. A primeira versão desta correção usava aspas e teria quebrado o reparo CHKDSK em toda máquina, inclusive nas que funcionam hoje — verificado empiricamente antes de ficar no diff.
  - Testes: `tests/unit/windowsSystemExecutables.test.cjs` — caminho absoluto existente e igual ao esperado, fallback para o nome simples quando o arquivo não existe, barras preservadas na expressão de elevação e tradução do ENOENT. Suíte completa: 150 testes, 0 falhas. Detalhamento em [`docs/bugs/closed/electron-main-spawn-powershell-enoent-lista-dispositivos_2026-09-06T12-00.md`](docs/bugs/closed/electron-main-spawn-powershell-enoent-lista-dispositivos_2026-09-06T12-00.md).

## [2.12.75] - 2026-09-06

### Added
- **Aviso de atualização na home para quem desligou a verificação automática (`HomePage.tsx`, `App.tsx`)**:
  - Complemento do crash corrigido na 2.12.72. Com `autoCheckUpdates` desligado, `checkForUpdates(false)` retorna cedo (`autoUpdateService.ts:141`) e o modal automático **nunca** aparece — a única rota até uma versão nova era o botão dentro de Configurações, justamente o que quebrava. Quem estava nessa combinação ficava sem nenhum caminho dentro do app.
  - Agora, quando a verificação automática está desativada, a home mostra uma barra âmbar dizendo isso, com **Verificar agora**. O botão chama `checkForUpdates(true)` — o `force` ignora tanto a preferência desligada quanto o intervalo de 12 h — e abre o mesmo `AppUpdateModal` pelo callback `onOpenUpdateModal`, já usado pelo `SettingsPage`. Nenhum canal IPC novo.
  - A barra só aparece com a preferência desligada, e a checagem só sai da máquina quando o usuário clica: a escolha de não verificar sozinho continua valendo.
  - **Ressalva registrada:** isto não resgata quem já está travado nas v2.12.43/2.12.58 — como a correção da 2.12.72, o código novo só existe depois de atualizar. O que salva esse grupo é o modal automático (`App.tsx`, 3,5 s após abrir), que nunca passou pelo `SettingsPage` e segue íntegro nas builds antigas. Para quem tem o auto-check desligado *e* está numa build antiga, a única saída continua sendo reinstalar manualmente.

## [2.12.74] - 2026-09-06

### Fixed
- **Atualização BadAvatar USB sem formatar preservava estado do console de origem**: a exclusão introduzida na 2.12.68 só filtrava a cópia e deixava dumps e bancos antigos no pendrive. A preparação agora compara tamanho e SHA-256 com o manifesto validado e move somente cópias idênticas para `.xbox-downloader/quarantine/foreign-console-state-<id>/`. Logs e bancos modificados, jogos, perfis e saves são preservados. A migração roda mesmo com diário concluído, retoma após interrupção e não sobrescreve quarentenas anteriores; no modo RGH, limita-se a `Aurora/`.
- Testes cobrem atualização de um plano já concluído, preservação de dados do usuário, interrupção, repetição e rejeição de caminhos inseguros. O bug **Fatal Crash Intercepted!** permanece aberto: a causa da exceção ainda depende de dumps do console afetado e validação física.

## [2.12.73] - 2026-09-06

### Fixed
- **Erro de rede era classificado como "Falha no dispositivo local" e abortava o fallback (`local_resilient.go`, `fallback.go`)**:
  - Print do usuário: *"Army of Two 2 The 40th Day — Erro — Falha no dispositivo local. O download concluído foi preservado para nova tentativa: HuggingFace download failed: fa…"*. O `fa` cortado é o começo de `falha no dispositivo local`, o texto de `ErrLocalDelivery` (o corte é da foto; `QueuePage.tsx` já quebra linha em erros desde a 2.12.71).
  - `huggingface.go:52` passa **todo** erro de download por `classifyLocalStorageFailure`, que marcava como `ErrLocalDelivery` bastando duas coisas: `connection.Mode == "local"` **e** o texto do erro conter um dos ~30 fragmentos de `isLikelyLocalStorageError`/`isLikelyLocalDeviceError`. `fallback.go` então escrevia o status e **retornava**, descartando `ia` e `minerva` — e a ordem padrão é `["huggingface", "ia", "minerva"]`, ou seja, o **primeiro** provedor derrubava a cadeia inteira.
  - O casamento é `strings.Contains` sobre a mensagem já concatenada, sem nenhuma noção de **onde** a falha ocorreu. No Windows um socket abortado aparece como `wsarecv: The specified network name is no longer available.` (ERROR_NETNAME_DELETED), que casa o fragmento `network name is no longer available`; `iaDownloadRange` devolve isso como `read after N bytes: %w` e `iaDownloadSingleAttempt` como `request failed: %w`. Uma queda de conexão com o provedor passava a abortar tudo — justamente a falha que outro provedor resolveria. Agora `isNetworkError` filtra esses erros antes da classificação, e eles caem para o próximo provedor.
  - O filtro testa os **tipos concretos** `*net.OpError`, `*net.DNSError` e `*url.Error` — **nunca a interface `net.Error`**. No Windows `syscall.Errno` declara `Timeout()`/`Temporary()`, então um `*os.PathError` de disco cheio *também* satisfaz `net.Error`; a primeira versão desta correção usava a interface e teria desligado a classificação de armazenamento inteira, deixando a cadeia baixar mais gigabytes no mesmo disco cheio e sem telemetria. Achado na revisão e confirmado empiricamente (`Errno(112)` dentro de `*os.PathError`: `net.Error` → `true`, tipos concretos → `false`). `TestIsNetworkErrorSeparatesDiskFromSocket` trava exatamente isso e falha com a versão de interface.
  - **A mensagem culpava o hardware errado.** Na fase `download-http` nada tocou o dispositivo de destino ainda: em modo local o arquivo vai para `ToolsDir/Ready/<nome>/.source_hf.<ext>` (`huggingface.go:44-47`, idem `pipeline.go:179-183`), sempre no drive do executável. O disco cheio é o **do PC**, e o usuário era mandado conferir o pendrive. Novo `ErrLocalStaging`, embrulhado junto de `ErrLocalDelivery` nas fases `download-*`, faz a mensagem nomear o disco certo e pedir espaço livre. O job continua parando — outro provedor também não cabe num disco cheio.
  - **"O download concluído foi preservado" era falso** nessa fase: nada concluiu. O parcial de fato sobrevive (nenhum cleanup roda nesse `return`, e `loadSingleResume` retoma depois), mas o texto sugeria uma falha *posterior* a um download bem-sucedido. Agora só a fase de gravação no destino — onde o download realmente concluiu — usa esse texto.
  - **Este caminho escapava da telemetria.** Os dois `return` de `ErrLocalDelivery` (`fallback.go:140` e `:172`) acontecem **antes** do `telemetry.Report` que a 2.12.71 adicionou no fim de `ProcessGameWithFallback` — por isso só restava a foto da tela, sem nenhum log. Ambos passam por `haltOnLocalStorageFailure`, que reporta levando `providerErrors` mais o erro do provedor que parou a cadeia.
  - Contexto que só apareceu na investigação: o catálogo "HuggingFace" é **80% archive.org** (1345 links contra 343 do `huggingface.co`) — `validHuggingFaceDownloadURL` valida só esquema e extensão, nunca o host. O título do print é um `.rar` de **5,22 GiB no archive.org**, então o download usa o caminho chunked paralelo e a label "HuggingFace" na mensagem de erro aponta a fonte errada. Registrado em [`docs/bugs/open/2026-09-06-fallback-abortado-por-erro-classificado-como-dispositivo-local.md`](docs/bugs/open/2026-09-06-fallback-abortado-por-erro-classificado-como-dispositivo-local.md), junto do defeito **não corrigido** aqui: em modo local o `archivePath` ignora a seleção do volume mais folgado (`bestFixedVolume`) e fica preso ao drive do app, porque é assim que ele sobrevive ao `cleanupGameScratch` entre tentativas de provedor.
  - **Não corrigido nesta versão** (registrado no doc do bug): as fases `extract-*` e `convert-god` também rodam no `TempDir` do PC — `outputRoot()` devolve `s.App.TempDir` incondicionalmente — mas continuam dizendo "Falha no dispositivo local", porque `stage_checkpoint.go:287,296` constroem `ErrLocalDelivery` direto, sem passar por `classifyLocalStorageFailure`. Também segue aberta a assimetria entre modos (em modo FTP o mesmo disco cheio não interrompe a cadeia).
  - Testes: `TestIsNetworkErrorSeparatesDiskFromSocket`, `TestClassifyLocalStorageFailureStillHaltsOnRealDiskError`, `TestClassifyLocalStorageFailureIgnoresNetworkErrors` e `TestClassifyLocalStorageFailureSeparatesStagingFromDevice`.

## [2.12.72] - 2026-09-06

### Fixed
- **Crash do renderer ao verificar atualizações (`SettingsPage.tsx`)**:
  - `Uncaught ReferenceError: onOpenUpdateModal is not defined` derrubava a interface inteira em Configurações. Achado na triagem da telemetria: eram os **3 únicos erros com status `open`** do projeto (ids **4850**, **4660**, **4361**), o mais recente em 2026-09-06 04:37, atingindo tanto a v2.12.43 quanto a v2.12.58 — ou seja, presente há pelo menos três releases.
  - A prop `onOpenUpdateModal` estava declarada na interface `SettingsPageProps` (linha 56), era passada por `App.tsx` (linha 391) e usada em três pontos (244, 245 e 784), mas **nunca foi desestruturada** dos props do componente. Em tempo de execução o identificador ficava livre, e ler um identificador não declarado lança `ReferenceError` — inclusive dentro da guarda `onOpenUpdateModal && onOpenUpdateModal(...)` da linha 784, que parece defensiva mas não protege contra isto (só protegeria se fosse uma propriedade `undefined`).
  - Consequência prática: o usuário clicava em *Verificar atualizações*, e **se houvesse uma versão nova** (o único caminho que chega à linha 244) a tela quebrava — justamente quem mais precisava atualizar não conseguia.
  - O `tsc` já acusava os quatro `TS2304: Cannot find name 'onOpenUpdateModal'`, mas o build do renderer usa Vite/esbuild, que **não faz checagem de tipos** — o erro passou direto para produção. Correção: uma linha, acrescentando `onOpenUpdateModal` à desestruturação.
...
- **Pânico no pipeline deixava de dizer o que quebrou, e não era reportado (`handlers.go`)**:
  - O `recover()` do `launcher` em `handleTrigger` gravava na fila do usuário o literal `"Server crashed during processing"` — a mensagem vai crua para a tela (`QueuePage.tsx` renderiza `job.message`), então o usuário via um jogo em erro sem nenhuma pista do motivo. Diferente do `RecoverMiddleware`, esse `recover()` também **não** chamava `telemetry.Report`, de modo que o pânico só existia no log da máquina do usuário.
  - Encontrado no log local de 2026-09-02: `PANIC processing Grand Theft Auto 5: runtime error: invalid memory address or nil pointer dereference`, com stack em `pipeline.(*Service).ProcessGameWithFallback` (`fallback.go:54`) — o mesmo caminho `source=unified` usado pelo Catálogo Online. A função foi reescrita em 2026-09-03 (90 inserções / 107 remoções); **não está confirmado** se o nil deref sobreviveu à reescrita.
  - Agora a fila mostra `Erro interno ao processar: <pânico>` e o caso é reportado como `http-server` / `handlers.go::handleTrigger.launcher`, com a stack anexada.
...
- **Revertida a remoção do `Aurora.Restart()` em `XboxCompanionReady.lua` (`readyToPlayConfiguration.ts`)**:
  ...
  - A hipótese de **laço de reinício** continua de pé e não foi descartada: se `ensureScanPath` voltar a reportar mudança a cada boot (divergência de `deviceid`, por exemplo), o reinício "uma única vez" se repete indefinidamente. Fica registrada em [`docs/bugs/closed/2026-09-05-fatal-crash-intercepted-pendrive-preparado.md`](docs/bugs/closed/2026-09-05-fatal-crash-intercepted-pendrive-preparado.md), em vez de justificar quebrar o recurso. O sinal do laço não está no `crashlog.txt` e sim em `Aurora\Data\Logs\debug.log`: mais de uma ocorrência de `Xbox 360 Companion: caminhos configurados; reiniciando Aurora.` por preparação.

## [2.12.68] - 2026-09-05

### Fixed
- **Diagnóstico do "Fatal Crash Intercepted!" no console (`readyToPlayConfiguration.ts`)**:
  - O banner *Fatal Crash Intercepted!* aparece ao ligar o Xbox com o pendrive preparado. **A autoria do texto não foi confirmada**: a string não existe em nenhum dos 643 arquivos do pacote (os executáveis do Aurora e do DashLaunch são comprimidos, então isso não a descarta). Dois manipuladores de crash rodam nesse ambiente e qualquer um poderia emiti-lo — o `exchandler` do DashLaunch, ligado por padrão quando ausente do `launch.ini`, e o do próprio Aurora, que é quem gera os `Data\Logs\*.crash.log`. O que os dois têm em comum: **algo lançou uma exceção não tratada**.
  - Investigar era impossível pelo lado do DashLaunch, porque o `launch.ini` gerado pelo app não definia `Dumpfile` e, sem ele, o texto da exceção sai **apenas pela UART**.
...
### Changed
- **Game processing now auto-runs on the roomiest internal drive (Windows)** — the entire per-game working set (torrent download → archive extraction → ISO→GOD conversion) is relocated off a cramped app/`GODSEND_HOME` partition. At startup the backend picks the fixed **NTFS/exFAT** drive with the most free space and runs both the download staging (`<drive>\godsend-temp\torrent-dl`) and processing scratch (`<drive>\godsend-temp\proc`) there. Removable and FAT32 volumes are skipped so multi-GB files never land on a pendrive or trip FAT32's 4 GB per-file limit; only the final split GOD/XEX files are written to the destination. Because download and processing now share one volume, the finished download no longer copies across drives. Falls back to `GODSEND_HOME/Temp` when no other drive is roomier (or on non-Windows). An explicit `GODSEND_TORRENT_TEMP` / Settings override still wins for the download staging.
- **Simple-mode setup wizard now asks unlock type before transfer method** - The USB/network choice moved into the guided flow, so users first choose the console unlock type, then choose between USB preparation and direct network transfer with back navigation between steps. When a prepared USB device is already connected, the wizard offers a direct jump to the game catalog instead.

### Fixed
- **Minerva torrent failures now report a real cause instead of "aria2c: exit status 9"** — aria2c's numeric exit codes are translated to human-readable reasons (exit 9 → "not enough disk space available", plus network/timeout/disk-I/O/torrent-parse codes). Previously the queue only showed `aria2c: exit status N`, and the captured "last output" was flooded by aria2c's periodic `*** Download Progress Summary ***` block, which buried the actual error line. The diagnostic tail now drops that summary noise so the surviving lines explain the failure.
- **Settings shows the real temp locations** — the *Temporary directories* screen now reports the paths the backend actually resolved (exposed via `GET /config` as `temp_dir` / `torrent_temp_dir`), so when processing auto-relocates to a roomier drive the UI reflects the true location instead of the stale app-drive default. Falls back to the computed default when the backend is unreachable.
- **Pre-flight disk-space check before torrent downloads** — the download now fails immediately with a clear message naming the *staging* volume and the setting to change (`torrent download temp` / `GODSEND_TORRENT_TEMP`) when that volume can't hold the selected file, instead of downloading for minutes and dying with the opaque exit-9. The check targets the drive the download actually writes to — which is **not** the destination shown in the UI, since games stage on the app/`GODSEND_HOME` drive and only the final split files reach the destination.
