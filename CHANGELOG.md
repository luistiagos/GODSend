...
- **Os cinco pontos restantes de extracao nunca invalidavam o arquivo corrompido, e a telemetria titulava o defeito com a mensagem errada (`services/pipeline/rom.go`, `minerva.go`, `fallback.go`) - v2.12.97**:
  - Fecha `docs/bugs/open/backend-pipeline-arquivo-corrompido-reaproveitado-do-cache_2026-09-12T12-24.md`. A 2.12.94 tinha ligado `invalidateDownloadedArchiveOnCorruptExtract` em HuggingFace, IA generico, IA digital e Redump/ISO, mas cinco chamadas de `extractArchiveResilient` ficaram de fora — e nelas o mesmo arquivo ruim continuava sendo reaproveitado a cada tentativa.
...
- **Falha ao criar a pasta de logs derrubava o app e impedia o backend de subir (`infrastructure/serverLog.ts`, `app/bootstrap.ts`) - v2.12.95**:
  - Fecha `docs/bugs/open/electron-main-falha-na-pasta-de-logs-impede-backend-de-subir_2026-09-13T15-59.md` (errors 6274/6275 e 6642/6643, duas maquinas em dois dias). O usuario via o dialogo "GODsend crashed" e o backend **nunca era iniciado** - porque nao deu para escrever uma linha de log.
...
- **Disco cheio do PC era relatado como "Falha no dispositivo local", e so parava a fila em modo pendrive (`services/pipeline/local_resilient.go`, `stage_checkpoint.go`, `fallback.go`) - v2.12.93**:
  - Fecha os itens 3, 6 e 8 de `docs/bugs/open/2026-09-06-fallback-abortado-por-erro-classificado-como-dispositivo-local.md`. Os itens 1, 2, 4 e 5 ja tinham sido fechados na 2.12.73; o item 7 continua aberto, agora com medicao nova (ver abaixo), e por isso o arquivo fica em `open/`.
...
- **O hook do Aurora podia reiniciar o console a cada boot do pendrive preparado (`infrastructure/readyToPlayConfiguration.ts`) - v2.12.92**:
  - Trabalha o candidato 2 de `docs/bugs/open/2026-09-05-fatal-crash-intercepted-pendrive-preparado.md`. O `Aurora.Restart()` do `XboxCompanionReady.lua` e deliberado - os scan paths so valem na inicializacao seguinte -, mas ele era disparado por `changed`, e nada garantia que `changed` fosse falso no boot seguinte. Um cadastro que se repetisse virava reinicio a cada boot: para o usuario, o console reiniciando ou travando logo ao inserir o pendrive.
