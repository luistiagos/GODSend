...
function sha256File(filePath) {
    try {
        return sha256(fs_1.default.readFileSync(filePath));
    }
    catch {
        return null;
    }
 ...
 */
function verifyCacheIntegrity(vdir, contentHashes) {
    if (!contentHashes || typeof contentHashes !== "object")
        return false;
    for (const [localName, expectedHash] of Object.entries(contentHashes)) {
        if (!expectedHash)
            continue; // file was absent at sync time
        const localPath = path_1.default.join(vdir, localName);
...
async function syncAuroraTitleVisualAssets(xboxIp, auroraRoot, titleId, gameDataDir, cacheRoot, force) {
            ...
            if (cachedFp && typeof cachedFp === "object") {
                ...
                if (allMatch) {
                    ...
                    if (verifyCacheIntegrity(vdir, prev?._contentHashes)) {
                        (0, backendClient_1.addOutputLine)(`[INFO] Visual sync ${titleId}: force refresh — fingerprints + hashes match, skipping.`);
                        return;
                    }
                    (0, backendClient_1.addOutputLine)(`[INFO] Visual sync ${titleId}: force refresh — hash mismatch, re-syncing.`);
                }
            }
        }
    ...
    async function cacheDecodedSlot(slotKey, pngBuf, localBase) {
        ...
        if (!force && fs_1.default.existsSync(lp)) {
            m[slotKey] = assetFor(localName);
            contentHashes[localName] = sha256File(lp) || "";
            return;
        }
        fs_1.default.writeFileSync(lp, pngBuf);
        m[slotKey] = assetFor(localName);
    ...
    // BK asset → background
    {
        ...
        if (buf && buf.length >= 2048) {
            ...
            if (decoded[4])
                await cacheDecodedSlot("background", decoded[4], "bk-background");
        }
    }
    // GC asset → cover
    {
        ...
        if (buf && buf.length >= 2048) {
            ...
            if (decoded[2])
                await cacheDecodedSlot("importCover", decoded[2], "gc-cover");
        }
    }
    // GL asset → icon + banner
    {
        ...
        if (buf && buf.length >= 2048) {
            ...
            if (decoded[0])
                await cacheDecodedSlot("icon", decoded[0], "gl-icon");
            if (decoded[1])
                await cacheDecodedSlot("banner", decoded[1], "gl-banner");
        }
    }
...
async function syncAuroraGameCoverAssets(xboxIp, auroraRoot, mediaDir, titleId, gameDataDir, cacheRoot, force) {
    ...
    if (force) {
        ...
        if (remoteSz >= 0 && fs_1.default.existsSync(binPath) && fs_1.default.statSync(binPath).size === remoteSz) {
            ...
            if (storedBinHash && localBinHash === storedBinHash) {
                ...
                if (prevMeta.primaryFile &&
                    fs_1.default.existsSync(path_1.default.join(gdir, prevMeta.primaryFile)) &&
                    prevMeta._coverHash) {
                    const localCoverHash = sha256File(path_1.default.join(gdir, prevMeta.primaryFile));
                    if (localCoverHash === prevMeta._coverHash) {
                        (0, backendClient_1.addOutputLine)(`[INFO] Cover sync ${titleId}: force refresh — hashes match, skipping.`);
    ...
    const binHash = fs_1.default.existsSync(binPath) ? sha256File(binPath) : null;
