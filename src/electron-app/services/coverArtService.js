...
exports.browseCoverCache = void 0;
exports.baseTitleForCover = baseTitleForCover;
exports.cleanTitleForSearch = cleanTitleForSearch;
exports.fetchXboxUnityCoverWithMeta = fetchXboxUnityCoverWithMeta;
exports.searchXboxUnityCovers = searchXboxUnityCovers;
exports.fetchMicrosoftStoreTitleIdForBrowse = fetchMicrosoftStoreTitleIdForBrowse;
exports.tryXboxCdnFromMicrosoftStoreSearch = tryXboxCdnFromMicrosoftStoreSearch;
exports.fetchWikipediaCover = fetchWikipediaCover;
exports.fetchXboxCdnAssets = fetchXboxCdnAssets;
exports.resolveTitleIdHex = resolveTitleIdHex;
const httpHelper_1 = require("../infrastructure/httpHelper");
// In-memory cache: base-title → { ok, dataUrl? }
exports.browseCoverCache = new Map();
...
async function fetchXboxUnityCoverWithMeta(searchTerm) {
    ...
    const jsonBuf = await (0, httpHelper_1.fetchHttpImage)(url);
    if (!jsonBuf || jsonBuf.length === 0)
        return null;
    let items;
    try {
        items = JSON.parse(jsonBuf.toString("utf8"));
    }
    catch {
        return null;
    }
    ...
    const buf = await (0, httpHelper_1.fetchHttpImage)(coverUrl);
    if (!buf || buf.length < 100)
        return null;
    return { buf, titleId };
}
async function searchXboxUnityCovers(searchTerm) {
    const url = `http://xboxunity.net/api/Covers/${encodeURIComponent(searchTerm)}`;
    const jsonBuf = await (0, httpHelper_1.fetchHttpImage)(url);
    if (!jsonBuf || jsonBuf.length === 0)
        return [];
...
async function fetchMicrosoftStoreTitleIdForBrowse(searchTerm) {
    ...
    let res;
    try {
        res = await fetch(`https://displaycatalog.mp.microsoft.com/v7.0/productFamilies/autosuggest?${p}`, { headers: { "User-Agent": "Mozilla/5.0 GODsend-browse-cover" } });
    }
    catch {
        return "";
    }
...
async function fetchWikipediaCover(articleTitle) {
    ...
    if (!imgUrl)
        return null;
    const buf = await (0, httpHelper_1.fetchHttpImage)(imgUrl);
    return buf && buf.length >= 100 ? buf : null;
}
async function fetchXboxCdnAssets(titleIdHex, locale = "en-US") {
    const result = {
        background: [], banner: [],
        icon: [], screenshot: [], cover: [],
    };
    if (!titleIdHex || !/^[0-9A-F]{8}$/i.test(titleIdHex))
        return result;
