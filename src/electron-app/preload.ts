...
contextBridge.exposeInMainWorld("godsendApi", {
  ...
  getEffectiveBackendTempPath: () => ipcRenderer.invoke("config:get-effective-backend-temp-path"),
  setTorrentTempPath:        (folder: string) => ipcRenderer.invoke("config:set-torrent-temp-path", folder),
  chooseTorrentTempPath:     () => ipcRenderer.invoke("config:choose-torrent-temp-path"),
