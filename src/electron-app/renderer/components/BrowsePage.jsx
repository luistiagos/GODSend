...
const DRIVES    = ["Hdd1:", "Usb0:", "Usb1:", "Usb2:", "Usb3:", "Usb4:"];
const METHODS   = [
  { id: "god",     label: "GOD",     desc: "ISO → Games on Demand" },
  { id: "content", label: "Content", desc: "Content folder (DLC tree)" },
...
function CoverArt({ dataUrl, name, size = 100 }) {
  return (
    <div
...
function QueueDialog({
  ...
  const [drive,  setDrive]  = useState(defaultDrive || DRIVES[0]);
  const [method, setMethod] = useState("god");
  const [queuing, setQueuing]   = useState(false);
  const [result,  setResult]    = useState(null); // {ok, status/error}
  const [discRec, setDiscRec]   = useState(null); // recommendation string
  ...
  useEffect(() => {
    ...
    window.godsendApi.browseGetDiscInfo(game).then((r) => {
...
export default function BrowsePage({ onBack }) {
  ...
  const [cacheProgress, setCacheProgress] = useState(null); // {loaded, total}
  const [filter,   setFilter]   = useState("");
  const [defaultDrive, setDefaultDrive] = useState("");

  const [selected,     setSelected]     = useState(null); // game name string when dialog open
  const [cover,        setCover]        = useState(undefined);
  const filterRef = useRef(null);

  // Load default drive once
  useEffect(() => {
    window.godsendApi.getDefaultXboxDrive().then((d) => {
  ...
  function openGame(name) {
    ...
    window.godsendApi.browseFetchCover(name).then((r) => {
