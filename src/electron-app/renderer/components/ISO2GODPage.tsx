    ...
    for (const file of files) {
      ...
      try {
        ...
        if (r.ok) {
          ...
          setFiles(prev => prev.map(f =>
            f.path === file.path ? { ...f, state: "error", error: r.error || "Falha na conversão" } : f
          ));
        }
      } catch (err: any) {
        setFiles(prev => prev.map(f =>
    ...
    <div className="flex flex-col h-full p-3 gap-2.5">
      ...
      <footer className="flex items-center gap-2 shrink-0">
        ...
        >
          ...
          {busy ? "Convertendo..." : `Converter ${pending.length} ISO${pending.length !== 1 ? "s" : ""} para GOD`}
        </Button>
        <span className="text-[11px] text-muted-foreground">
          Saída: Nome do título - TitleID
        </span>
      </footer>
    </div>
  );
