...
function register(ipcMain) {
    ...
    ipcMain.handle("xbox:fetch-aurora-covers", async (_event, gameList, opts) => {
        ...
        let lastProcessed = -1;
        try {
            (0, backendClient_1.addOutputLine)(`[INFO] Aurora covers + artwork: FTP sync starting for ${gameList.length} title(s)…`);
            ...
            (0, backendClient_1.addOutputLine)(`[INFO] Aurora covers + artwork: finished ${gameList.length} title(s).`);
            return { ok: true };
        }
        catch (err) {
            const msg = err.message || String(err);
